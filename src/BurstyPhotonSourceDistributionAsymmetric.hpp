 ...
 */
#ifndef BURSTYPHOTONSOURCEDISTRIBUTIONASYMMETRIC_HPP
#define BURSTYPHOTONSOURCEDISTRIBUTIONASYMMETRIC_HPP
 ...
 */
class BurstyPhotonSourceDistributionAsymmetric : public PhotonSourceDistribution {
private:
...
// const double bursty_star_formation_rate; 
  ...
  // end of mgb edit 
  const double _asymmetric_offset;
  /*! @brief asymmetric offset */
...
public:
   ...
   */
  inline BurstyPhotonSourceDistributionAsymmetric(
      ...
      const double asymmetric_offset, // mgb edit 07.11.2025
    ...
    //  const bool moving_sources_flag, // mgb 16.10.2025
      ...
      : _star_formation_rate(star_formation_rate), _bursty_sfr_flag(bursty_sfr_flag), _length_of_burst(length_of_burst), _time_of_burst_peak(time_of_burst_peak),
        _burst_amplitude(burst_amplitude), _asymmetric_offset(asymmetric_offset), _peakiness_fraction(peakiness_fraction),
        _scale_height_peak(scale_height_peak), _type1_sne_scale_height(type1_sne_scale_height), _type1_flag(type1_flag),
        _kennicutt_schmidt_index(kennicutt_schmidt_index),
    ...
    if (output_sources) {
      ...
      *_output_file2 << "time (s)\tlum (s^-1)\tnumsne\tSFR (Msol Myr-1)\tSFR (Msol Myr^-1 kpc^-2)\tSFR (kg s^-1 kpc^-2)\n";
      _output_file2->flush();
   ...
   */
  BurstyPhotonSourceDistributionAsymmetric(ParameterFile &params, Log *log = nullptr)
      : BurstyPhotonSourceDistributionAsymmetric(
            ...
            params.get_physical_value< QUANTITY_BURST_AMPLITUDE >(
                "PhotonSourceDistribution:burst amplitude", "5.0 Msol yr^-1"), // edit mgb 19.09.2025
            params.get_physical_value< QUANTITY_ASYMMETRIC_OFFSET >( // edit mgb
                "PhotonSourceDistribution:asymmetric offset", "0 pc"),
   ...
   */
  virtual ~BurstyPhotonSourceDistributionAsymmetric() {}
...
///   Type Ia supernovae implementation - up to 325 pc scale height
      ...
      while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          for (auto it = subgrid.hydro_begin(); it != subgrid.hydro_end();
            ...
            double cell_z = it.get_cell_midpoint()[2]; // std::abs(it.get_cell_midpoint()[2]); mgb edit 11.11.2025

            if (cell_z > (_scale_height_peak+_asymmetric_offset) || cell_z < (_asymmetric_offset-_scale_height_peak)) { // asymmetric offset mgb 11.10.2025 6.171e18
              cell_mass = 0.0;
            }
    ...
    //  if ((_total_time >= _time_of_burst_start) && (_total_time <= (_time_of_burst_start + _length_of_burst))) {
      //   _bursty_star_formation_rate = _star_formation_rate * _burst_amplitude;
       // double mass_to_generate = _update_interval*_bursty_star_formation_rate/1.988e30*0.207*(std::pow(running_mass/init_running_mass,1.4)); // _star_formation_rate
      //} else {
        // _bursty_star_formation_rate = _star_formation_rate;
     // }
     ...
     if (_bursty_sfr_flag == true) {
      _bursty_star_formation_rate = _star_formation_rate_area + (_burst_amplitude*_star_formation_rate_area - _star_formation_rate_area) * exp(-pow(((_total_time - _time_of_burst_peak)/_length_of_burst), 2)); // edit mgb 19.09.2025 should be in units kg s^-1. kpc^-2
     } else {
      ...
      double mass_to_generate = _update_interval*_bursty_star_formation_rate/unit_Msol*0.207*(std::pow(running_mass/init_running_mass, _kennicutt_schmidt_index)); // _star_formation_rate replaced 1.988e30 with unit_Msol mgb edit 12.11.2025 

      if (_output_file2 != nullptr) {
        double totallum = get_total_luminosity();
        *_output_file2 << _total_time << "\t" << totallum << "\t" << _num_sne << "\t" << ((_bursty_star_formation_rate*unit_Myr)/(unit_Msol)) << "\t" << ((_bursty_star_formation_rate*unit_Myr)/(area_kpc*unit_Msol)) << "\t" <<  ((_bursty_star_formation_rate/area_kpc)*std::pow(running_mass/init_running_mass,1.4)) << "\n"; // output the SFR in Msol Myr^-1 kpc^-2 and in kg s^-1 kpc^-2
        _output_file2->flush();
...
// --------------------------------------
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    restart_writer.write(_asymmetric_offset); // mgb edit 07.11.2025
   ...
   */
  inline BurstyPhotonSourceDistributionAsymmetric(RestartReader &restart_reader)
      ...
      //  _moving_sources_flag(restart_reader.read< bool >())
        ...
        _asymmetric_offset(restart_reader.read< double >()), // mgb edit 07.11.2025 
...
#endif // BURSTYPHOTONSOURCEDISTRIBUTIONASYMMETRIC_HPP
