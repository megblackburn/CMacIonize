...
public:
  ...
  inline void do_flux_calculation(const uint_fast8_t i,
    ...
    cmac_assert_message(fluxfac >= 0. && fluxfac <= 1., "fluxfac3: %g", fluxfac);
    {
      ...
      if (pn2 * right_state.get_primitives_density() > // mgb edit 04.08.2026 - changed p2 to pn2
   ...
   /* if (dvxdy != 0 || dvxdz != 0){
    ...
    if (_revert_to_1d_hydro) {
        v_new = CoordinateVector<>(
            vx - dt * (vx * divv + rhoinv * dPdx - ax), 
            vy - dt * (vy * divv + rhoinv * dPdy - ay),
            vz - dt * (vz * divv + rhoinv * dPdz - az)
        );
    } else {
        v_new = CoordinateVector<>(
            vx - dt * (vx * dvxdx + vy * dvxdy + vz * dvxdz + rhoinv * dPdx - ax),
            vy - dt * (vx * dvydx + vy * dvydy + vz * dvydz + rhoinv * dPdy - ay),
            vz - dt * (vx * dvzdx + vy * dvzdy + vz * dvzdz + rhoinv * dPdz - az)
        );
    }
