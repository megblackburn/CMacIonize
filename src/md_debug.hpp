...
public:
  ...
  inline MixedDrivingPhotonSourceDistribution(
    ...
    if (_log) {
          _log->write_status(
              "After file writes. before read_file", _read_file);
      }

    if (_read_file){
      if (_log) {
          _log->write_status(
              "Inside read_file", _read_file);
      } 
   ...
   if (_log) {
          _log->write_status(
              "No source_luminosity from first loop adding ostars.", _source_luminosities.size());
        }   
  }
  ...
  virtual double get_total_luminosity() const {
    double tot_lum = 0.0;
    double tot_min_limit = 0.0; // mgb
    if (_log) {
        _log->write_status(
            "Within call to get_total_luminosity ", _source_luminosities.size());
      }
    for (uint_fast32_t i=0;i<_source_luminosities.size();++i) {
      ...
      if (_log) {
          _log->write_status(
              "No source_luminosity.", _source_luminosities[i], "i = ", i);
        }  
        
    }
    if (tot_lum < tot_min_limit) { // mgb
      if (_log) {
          _log->write_warning(
              "No total_luminosity.", tot_lum);
        } 
      }
    return tot_lum;
  }
   ...
   */
   virtual bool update(DensitySubGridCreator< HydroDensitySubGrid > *grid_creator, double actual_timestep) override {
    ...
    int should_have_done = int(4*area_kpc*_total_time/3.15576e13)*0; // what is this doing mgb 
    ...
    if (_total_time - _last_sf > _update_interval) {
      ...
      // 0.073 factor is to take into account we only form stars over 8Msol - where is this factor used? mgb
