...
/**
 * @file SteadySFRPhotonSourceDistribution.hpp
 *
 * @brief SteadySFRPhotonSourceDistribution.
 *
 * @author Meg Blackburn (mgb27@st-andrews.ac.uk)
 */
#ifndef STEADYSFRPHOTONSOURCEDISTRIBUTION_HPP
#define STEADYSFRPHOTONSOURCEDISTRIBUTION_HPP
 ...
 */
class SteadySFRPhotonSourceDistribution : public PhotonSourceDistribution {
private:
...
public:
   ...
   */
  inline SteadySFRPhotonSourceDistribution(
    ...
    if (output_sources) {
      if (_restart_flag == true) {
        ...
        *_output_file2 << "time (s)\tlum (s^-1)\tnumsne\tSFR_base (Msol Myr-1)\tSFR_KS (Msol Myr^-1 kpc^-2)\tSFR_KS (kg s^-1 kpc^-2)\n";
        _output_file2->flush(); 
      } else {
        _output_file = new std::ofstream(_source_filename);
        *_output_file << "#time (s)\tx (m)\ty (m)\tz (m)\tevent\tindex\tluminosity\tMass\ttype\n";
   ...
   */
  SteadySFRPhotonSourceDistribution(ParameterFile &params, Log *log = nullptr)
      : SteadySFRPhotonSourceDistribution(
   ...
   */
  virtual ~SteadySFRPhotonSourceDistribution() {}
...
// --------------------------------------
   ...
   */
  inline SteadySFRPhotonSourceDistribution(RestartReader &restart_reader)
...
#endif // STEADYSFRPHOTONSOURCEDISTRIBUTION_HPP
