 ...
 * @brief Factory for DensityGridWriter implementations.
 */
class DensityGridWriterFactoryMHD {
public:
  /**
   ...
   */
  static DensityGridWriterMHD *generate(std::string output_folder,
                                     ParameterFile &params,
                                     const bool hydro = false,
                                     Log *log = nullptr) {
    std::cout << "Within density grid writer factory Line 89" << std::endl;
    ...
    std::cout << "After requested grid writer factory Line 96" << std::endl;
#ifndef HAVE_HDF5
    check_hdf5(type, log);
#endif
...
#ifdef HAVE_HDF5
    if (type == "Gadget") {
      std::cout << "Gadget requested Line 89" << std::endl;
      return new GadgetDensityGridWriterMHD(output_folder, params, hydro, log);
#endif
    } else {
