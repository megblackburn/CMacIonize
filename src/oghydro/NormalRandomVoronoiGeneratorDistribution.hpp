...
/**
 * @file NormalRandomVoronoiGeneratorDistribution.hpp
 *
 * @brief Normal random VoronoiGeneratorDistribution implementation.
 *
 * @author Bert Vandenbroucke (bv7@st-andrews.ac.uk)
 */
#ifndef NORMALRANDOMVORONOIGENERATORDISTRIBUTION_HPP
#define NORMALRANDOMVORONOIGENERATORDISTRIBUTION_HPP
 ...
 */
class NormalRandomVoronoiGeneratorDistribution
...
public:
   ...
   */
  NormalRandomVoronoiGeneratorDistribution(
    ...
    if (log) {
      log->write_status(
          "Created NormalRandomVoronoiGeneratorDistribution with ",
   ...
   * @param log Log to write logging info to.
   */
  NormalRandomVoronoiGeneratorDistribution(const Box<> &simulation_box,
  ...
  virtual CoordinateVector<> get_position() {
    ...
    bool got_z = false;
    while (!got_z) {
      pos[2] = 0.1*_box.get_sides().z()*std::log(_random_generator.get_uniform_random_double());
      if (_random_generator.get_uniform_random_double() < 0.5) {
        pos[2] = -1.*pos[2];
      }
      if (pos[2] > _box.get_anchor().z() + _box.get_sides().z() || pos[2] < _box.get_anchor().z()) {
        got_z = false;
      } else {
        got_z = true;
      }
