 ...
 * executed for every cell of the DensityGrid, possibly in parallel.
 */
template < typename _function_, typename GridType> class DensityGridTraversalJobMarket {
private:
...
public:
  ...
  inline DensityGridTraversalJobMarket(
      GridType &grid, _function_ &function,
   ...
   * instance.
   */
  inline DensityGridTraversalJob< _function_ , GridType> *
  get_job(int_fast32_t thread_id) {
    ...
    std::pair< typename GridType::iterator, typename GridType::iterator > chunk =
        _grid.get_chunk(begin, end);
    DensityGridTraversalJob< _function_, GridType > *job =
        new DensityGridTraversalJob< _function_, GridType >(chunk.first, chunk.second,
                                                  _function);
    return job;
  }
};
