 ...
 */
#ifndef DENSITYGRIDTRAVERSALJOBMARKETMHD_HPP
#define DENSITYGRIDTRAVERSALJOBMARKETMHD_HPP

#include "DensityGridMHD.hpp"
#include "DensityGridTraversalJobMHD.hpp"
#include "Lock.hpp"
#include "Timer.hpp"
 ...
 * executed for every cell of the DensityGrid, possibly in parallel.
 */
template < typename _function_ > class DensityGridTraversalJobMarketMHD {
private:
...
public:
   ...
   */
  inline DensityGridTraversalJobMarketMHD(
      DensityGridMHD &grid, _function_ &function,
   ...
   * instance.
   */
  inline DensityGridTraversalJobMHD< _function_ > *
  get_job(int_fast32_t thread_id) {
    ...
    std::pair< DensityGridMHD::iterator, DensityGridMHD::iterator > chunk =
        _grid.get_chunk(begin, end);
    DensityGridTraversalJobMHD< _function_ > *job =
        new DensityGridTraversalJobMHD< _function_ >(chunk.first, chunk.second,
                                                     _function);
    return job;
  }
};

#endif // DENSITYGRIDTRAVERSALJOBMARKETMHD_HPP
