...
/**
 * @file TaskBasedRadiationMagnetoHydrodynamicsSimulation.cpp
 *
...
make_hydro_tasks(ThreadSafeVector< Task > &tasks, const uint_fast32_t igrid,
                 DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid > &grid_creator) {

  MagnetoHydroDensitySubGrid &this_grid = *grid_creator.get_subgrid(igrid);
  ...
  { // mgb edit 28.05.2026
    size_t bridge1_idx = tasks.get_free_element();
    Task &bridge1 = tasks[bridge1_idx];
    bridge1.set_type(TASKTYPE_SLOPE_LIMITER); // Re-uses the limiter execution state safely
    bridge1.set_subgrid(igrid);
    bridge1.set_dependency(this_grid.get_dependency()); // Primary: Waits for local grid loops
    if (ngbx != NEIGHBOUR_OUTSIDE) {
      bridge1.set_extra_dependency((*grid_creator.get_subgrid(ngbx)).get_dependency());
    }

    size_t bridge2_idx = tasks.get_free_element();
    Task &bridge2 = tasks[bridge2_idx];
    bridge2.set_type(TASKTYPE_SLOPE_LIMITER);
    bridge2.set_subgrid(igrid);
    bridge2.set_dependency(this_grid.get_dependency()); // Primary: Locked behind Bridge 1 finishing
      if (ngby != NEIGHBOUR_OUTSIDE) {
        bridge2.set_dependency((*grid_creator.get_subgrid(ngby)).get_dependency());
      } else {
        bridge2.set_dependency(this_grid.get_dependency());
      }
      if (ngbx != NEIGHBOUR_OUTSIDE) {
        bridge2.set_extra_dependency((*grid_creator.get_subgrid(ngbx)).get_dependency());
      }
  }
  ...
  // primitive variable prediction
  {
    ...
    if (ngbz != NEIGHBOUR_OUTSIDE) {
      task.set_extra_dependency((*grid_creator.get_subgrid(ngbz)).get_dependency());
    }
...
set_dependencies(const uint_fast32_t igrid,
  ...
  const MagnetoHydroDensitySubGrid &this_grid = *grid_creator.get_subgrid(igrid);
...
inline void reset_hydro_tasks(ThreadSafeVector< Task > &tasks,
                              MagnetoHydroDensitySubGrid &this_grid) {
...
execute_task(const size_t itask,
             ...
             const MagnetoHydro &hydro, const MagnetoHydroBoundaryManager &boundary_manager) {

  const Task &task = tasks[itask];
  MagnetoHydroDensitySubGrid &subgrid = *grid_creator.get_subgrid(task.get_subgrid());
 ...
 * options.
 */
void TaskBasedRadiationMagnetoHydrodynamicsSimulation::add_command_line_parameters(
...
while (clock < total_dt) {
  ...
  //if (ionization_variables.get_temperature() > 1e8) {
   // cmac_warning("A cell's temperature went above 1e8 K in the heating/cooling function. Etot = %g, T=%g, h0=%g, ne=%g, Bx = %g, By = %g, Bz = %g", hydro_variables.get_conserved_total_energy(), ionization_variables.get_temperature(), ionization_variables.get_ionic_fraction(ION_H_n), ionization_variables.get_number_density(), hydro_variables.get_primitives_magnetic_field().x(), hydro_variables.get_primitives_magnetic_field().y(), hydro_variables.get_primitives_magnetic_field().z());
 ...
 // }
  ...
  if (std::isnan(ionization_variables.get_temperature())) {
    cmac_warning("A cell's temperature went to NaN in the heating/cooling function.")
  }
 ...
 temp = ionization_variables.get_temperature();
  ...
  // mgb debug 21.05.2026
  double temp_from_eint = hydro_variables.get_conserved_internal_energy()/e_factor;

  CoordinateVector<>B_field = hydro_variables.get_conserved_magnetic_field() * inverse_volume;
  ...
  double emag = (1./inverse_volume) *0.5 * (Bx*Bx + By*By + Bz*Bz)/mu0;

  double eint_from_tot = hydro_variables.get_conserved_total_energy()-ekin-emag;
 ...
 // if (hydro_variables.get_conserved_internal_energy() != eint0){
  ...
  if (gain != gain || loss != loss){
   ...
   // mgb comment 21.05.2026 - this is not in line with current internal energy (tracker or total)
/*
  if (std::abs(temp) > 10*std::abs(temp_from_eint) || std::abs(temp) < 0.9*std::abs(temp_from_eint)){
    std::cout<< "Warning: Temperature is misaligned from internal energy! ionization temp: " << temp << ", internal energy temp: " << temp_from_eint << ", total energy temp: " << temp_from_etot << std::endl; 
  }

  current_energy = e_factor*temp; // mgb comment 21.05.2026 - this is not in line with current internal energy (tracker or total)

  if (current_energy > 100*hydro_variables.get_conserved_internal_energy()){
    std::cout << "Warning: Internal Energies are misaligned (higher than actual)! ionization Eint: " << current_energy << ", internal Eint: " << hydro_variables.get_conserved_internal_energy() << ", eint from tot: " << eint_from_tot << std::endl; 
    std::cout << "Net Gain/Loss: " << gain-loss << "Gain: " << gain << "Loss: " << loss << "time left: " << time_left << std::endl;
    std::cout << "dE limit: " << max_frac * current_energy << std::endl;
  }
  if (10*current_energy < hydro_variables.get_conserved_internal_energy()){
    std::cout << "Warning: Internal Energies are misaligned (lower than actual)! ionization Eint: " << current_energy << ", internal Eint: " << hydro_variables.get_conserved_internal_energy() << ", eint from tot: " << eint_from_tot << std::endl; 
    std::cout << "Net Gain/Loss: " << gain-loss << "Gain: " << gain << "Loss: " << loss << "time left: " << time_left << std::endl;
    std::cout << "dE limit: " << max_frac * current_energy << std::endl;
  }
    */
  ...
  if (hydro_variables.get_conserved_internal_energy()+dE < 0){ // mgb comment 22.05.2026: this should still allow greater gains in internal energy but will limit the losses
    std::cout << "Temperature 100*Eint<Eint_new! Eint: " << hydro_variables.get_conserved_internal_energy() << ", ionised Eint: " << current_energy << ", dE: " << dE << std::endl;
    std::cout << "Ionised Temp: " << temp << ", Eint Temp: " << temp_from_eint << ", Etot Temp: " << temp_from_etot << std::endl;
    
  }

  if (hydro_variables.get_conserved_internal_energy() > current_energy * 100){ // mgb comment 22.05.2026 this should boost gains and losses if Eint > Eint from radiation
    std::cout << "FATAL Eint > 100*Eint_new! Eint: " << hydro_variables.get_conserved_internal_energy() << ", ionised Eint: " << current_energy  << ", dE: " << dE << std::endl;
    std::cout << "Ionised Temp: " << temp << ", Eint Temp: " << temp_from_eint << ", Etot Temp: " << temp_from_etot << std::endl;
  ...
  if (std::isnan(hydro_variables.get_conserved_total_energy()) || std::isnan(dE) || std::isnan(hydro_variables.get_conserved_internal_energy())) {
    cmac_warning("Total energy is NaN before energy update in do_explicit_heat_cool! total energy: %g, internal energy: %g, dE=%g", hydro_variables.get_conserved_total_energy(), hydro_variables.get_conserved_internal_energy(), dE);
  }
  if (hydro_variables.get_conserved_total_energy() < hydro_variables.get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within do_explicit_heat_cool before energy update! Etot: " << hydro_variables.get_conserved_total_energy() << ", Eint: " << hydro_variables.get_conserved_internal_energy() << std::endl; 
            }
  if (std::abs(dE) > hydro_variables.get_conserved_internal_energy()){
        std::cout << "Warning: do_explicit_heat_cool, Eint < dE: " << hydro_variables.get_conserved_internal_energy() << ", dE: " << dE << std::endl;
      }
  hydro.update_energy_variables(ionization_variables, hydro_variables, inverse_volume, dE);
  if (hydro_variables.get_conserved_total_energy() < hydro_variables.get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within do_explicit_heat_cool after energy update! Etot: " << hydro_variables.get_conserved_total_energy() << ", Eint: " << hydro_variables.get_conserved_internal_energy() << std::endl; 
            }
  //if (hydro_variables.get_conserved_internal_energy() != eint0){
  //  std::cout << "Internal energy has been updated within update_energy_variables, Eint0: " << eint0 << ", Eint" << hydro_variables.get_conserved_internal_energy() << std::endl;
 // }
  if (std::isnan(hydro_variables.get_conserved_total_energy()) || std::isnan(hydro_variables.get_conserved_internal_energy())) {
    cmac_warning("Total energy is NaN after energy update in do_explicit_heat_cool! total energy: %g, internal energy: %g, dE=%g", hydro_variables.get_conserved_total_energy(), hydro_variables.get_conserved_internal_energy(), dE);
  }
...
//loss = _line_cooling_data.get_cooling(T, ne, abund) * n * V;
  ...
  double abund[LINECOOLINGDATA_NUMELEMENTS] = {0.0};
  ...
  CoordinateVector<>B = hydro_variables.get_conserved_magnetic_field(); // mgb edit 21.05.2026

  double _mu0 = PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_MAGNETC_PERMEABILITY); // mgb edit 21.05.2026

  double magnetic_energy = 0.5 * CoordinateVector<>::dot_product(B,B)/_mu0; // mgb edit 21.05.2026

  double bkp_thermal = hydro_variables.get_conserved_total_energy() - kinetic_energy-magnetic_energy; // mgb edit 21.05.2026
  ...
  double t_from_eint = hydro_variables.get_conserved_internal_energy()/e_factor;
  std::cout<< "Warning: Temperature set in do_cooling: shouldn't be happening, T = " << temperature << ", T from Eint: " << t_from_eint << std::endl;
 ...
 */
int TaskBasedRadiationMagnetoHydrodynamicsSimulation::do_simulation(
  ...
  DensityFunctionMHD *density_function =
      DensityFunctionFactoryMHD::generate(*params, log);
  ...
  if (params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:diffuse field", false)) {
  ...
  if (params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:external gravity",
          false)) {
    external_potential = ExternalPotentialFactory::generate(*params, log);
  }
  MagnetoHydroMask *hydro_mask = nullptr;
  if (params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:use mask", false)) {
    if (restart_reader == nullptr) {
      hydro_mask = MagnetoHydroMaskFactory::generate(*params, log);
    } else {
      hydro_mask = MagnetoHydroMaskFactory::restart(*restart_reader, log);
    }
  ...
  const double hydro_total_time = params->get_physical_value< QUANTITY_TIME >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:total time", "1. s");

  double hydro_minimum_timestep = params->get_physical_value< QUANTITY_TIME >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:minimum timestep", "-1. s");
  ...
  double hydro_maximum_timestep = params->get_physical_value< QUANTITY_TIME >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:maximum timestep", "-1. s");
  ...
  double hydro_snaptime = params->get_physical_value< QUANTITY_TIME >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:snapshot time", "-1. s");
  if (hydro_snaptime < 0.) {
  ...
  const uint_fast32_t hydro_firstsnap = params->get_value< uint_fast32_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:first snapshot", 0);

  const double hydro_radtime = params->get_physical_value< QUANTITY_TIME >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:radiation time", "-1. s");
  uint_fast32_t hydro_lastrad = 0;
  double lastrad_time = 0.0;
  const bool do_radiation = params->get_value< bool >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:do radiation", true);


  const bool do_self_gravity = params->get_value< bool>(
         "TaskBasedRadiationMagnetoHydrodynamicsSimulation:do self gravity",false);
  const double self_grav_theta = params->get_value<double>(
       "TaskBasedRadiationMagnetoHydrodynamicsSimulation:self gravity theta", 0.5);
  ...
  const bool _throttle_ion_state = params->get_value< bool >(
     "TaskBasedRadiationMagnetoHydrodynamicsSimulation:throttle ion state", false);
  ...
  const bool _restart_flag = params->get_value< bool >( // mgb edit 10.11.2025
    "TaskBasedRadiationMagnetoHydrodynamicsSimulation:restart flag", false
  );

  const double _restart_time = params->get_value< double >( // mgb edit 10.11.2025
    "TaskBasedRadiationMagnetoHydrodynamicsSimulation:restart time", 0.0
  );

  const double _restart_iteration = params->get_value< double >( // mgb edit 10.11.2025
    "TaskBasedRadiationMagnetoHydrodynamicsSimulation:restart iteration", 0.0
  );
 ...
 // const double _temperature_to_trace = params->get_physical_value< QUANTITY_TEMPERATURE >("DensityFunction:temperature to trace", "500. K");
  ...
  const bool _time_dependent_ionization = params->get_value<bool> ( 
    "TaskBasedRadiationMagnetoHydrodynamicsSimulation:time dependent ionization", false);
#ifndef HAVE_GSL
 ...
 // }
  ...
  if (_throttle_ion_state || _time_dependent_ionization) {
    maximum_neutral_fraction = 1.e-3;
      ...
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:maximum neutral fraction",
      -1.);
  }
  ...
  const size_t number_of_buffers = params->get_value< size_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:number of buffers", 50000);
  const size_t queue_size_per_thread = params->get_value< size_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:queue size per thread", 10000);
  const size_t shared_queue_size = params->get_value< size_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:shared queue size", 100000);
  const size_t number_of_tasks = params->get_value< size_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:number of tasks", 500000);
  int_fast32_t random_seed = params->get_value< int_fast32_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:random seed", 42);
  if (restart_reader != nullptr) {
    random_seed = restart_reader->read< int_fast32_t >();
  }
  time_logger.start("density grid creation");
  DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid > *grid_creator = nullptr;
  if (restart_reader == nullptr) {
    grid_creator = new DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid >(
        simulation_box.get_box(), *params);
  } else {
    grid_creator =
        new DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid >(*restart_reader);
  }
  time_logger.end("density grid creation");

  AlveliusTurbulenceForcing *turbulence_forcing = nullptr;
  if (params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:turbulent forcing",
          false)) {
    if (restart_reader == nullptr) {
  ...
  const bool do_rad_cool = params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:do radiative cooling",
          false);

  const bool use_cool_tables = params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:use cooling tables",
          false);
  

  const bool do_explicit_temp_calc = params->get_value< bool >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:do explicit temperature calculation",
          false);

  if (do_rad_cool || do_explicit_temp_calc) {
    _cooling_temp_floor = params->get_physical_value< QUANTITY_TEMPERATURE >(
            "TaskBasedRadiationMagnetoHydrodynamicsSimulation:cooling temperature floor",
            "10 K");
  ...
  double _gamma = params->get_value< double >("MagnetoHydro:polytropic index", 5. / 3.);
  double _mu0 = PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_MAGNETC_PERMEABILITY);

  const bool do_stellar_feedback = params->get_value< bool >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:do stellar feedback", false);
  ...
  std::string output_folder =
      Utilities::get_absolute_path(params->get_value< std::string >(
          "TaskBasedRadiationMagnetoHydrodynamicsSimulation:output folder", "."));
  DensityGridWriterMHD *writer =
      DensityGridWriterFactoryMHD::generate(output_folder, *params, true, log);

  uint_fast32_t nloop = params->get_value< uint_fast32_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:number of iterations", 10);

  uint_fast64_t numphoton = params->get_value< uint_fast64_t >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:number of photons", 1e6);

  const double _max_photon_distance = params->get_physical_value< QUANTITY_LENGTH >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:max photon distance", "-1 kpc");

  const double CFL = params->get_value< double >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:CFL", 0.2);

  const double source_copy_level = params->get_value< double >(
      "TaskBasedRadiationMagnetoHydrodynamicsSimulation:source copy level", 4);
  ...
  MagnetoHydro hydro(abundances, *params);
  MagnetoHydroBoundaryManager hydro_boundary_manager(*params);
...
//  std::cout << "After task-based initialization statement Line 1782" << std::endl;
  if (log) {
    log->write_status("Allocating memory space...");
  }
//  std::cout << "After memory allocation statement Line 1786" << std::endl;
...
#pragma omp parallel default(shared)
    ...
    while (igrid.value() < grid_creator->number_of_original_subgrids()) {
      const size_t this_igrid = igrid.post_increment();
      if (this_igrid < grid_creator->number_of_original_subgrids()) {
        MagnetoHydroDensitySubGrid &subgrid = *grid_creator->get_subgrid(this_igrid);
  ...
  if (do_self_gravity) {
    ...
    BarnesHutTreeMHD tree(simulation_box.get_box().get_sides(),simulation_box.get_box().get_anchor(),self_grav_theta);
...
// good reference point mgb 
    while (igrid.value() < grid_creator->number_of_original_subgrids()) {
      ...
      if (this_igrid < grid_creator->number_of_original_subgrids()) {
        ...
        for (auto cellit = (*gridit).hydro_begin();
          ...
          if (std::isnan(cellit.get_hydro_variables().get_conserved_total_energy()) || std::isnan(cellit.get_hydro_variables().get_conserved_internal_energy())) {
              cmac_warning("NaN energy in subgrid %zu after stellar feedback momentum injection!", this_igrid);
            }    
            const double dE = cellit.get_hydro_variables().get_energy_term(); // energy term after supernova update
            
            if (dE > 0.0) { // if resolved this should be greater than zero.
            //  if (std::abs(dE) > cellit.get_hydro_variables().get_conserved_internal_energy()){
            //    std::cout << "Warning: supernova restart, Eint < dE: " << cellit.get_hydro_variables().get_conserved_internal_energy() << ", dE: " << dE << std::endl;
              //}
              double energy_before = cellit.get_hydro_variables().get_conserved_total_energy(); 
              hydro.update_energy_variables(cellit.get_ionization_variables(),
                                            cellit.get_hydro_variables(),
                                            1. / cellit.get_volume(), dE);
              double energy_after = cellit.get_hydro_variables().get_conserved_total_energy();  
              if (std::isnan(energy_after)) {
                cmac_warning("Energy NaN in stellar feedback! Before: %g, after: %g", energy_before, energy_after);
              }
          ...
          //}
          if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
            std::cout<<"Warning Etot < Eint within stellar feedback loop before update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
          }
          ...
          if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
            std::cout<<"Warning Etot < Eint within stellar feedback loop after update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
          }
        //  if (cellit.get_ionization_variables().get_temperature() > 9.9999e8) {
        //    hydro.set_temperature(cellit.get_ionization_variables(), cellit.get_hydro_variables(),
        //            cellit.get_volume(), 9.9999e8);Warning Etot < Eint within ionization update before update!
        //  }
  ...
  if (restart_reader == nullptr) {
    ...
    {
     // std::cout << "Within First Time step Line 2018" << std::endl; // mgb comment 28.04.2026
     ...
     // std::cout << "requested time step list Line 2023" << std::endl; // mgb comment 28.04.2026
      AtomicValue< size_t > igrid(0);
      start_parallel_timing_block();
#ifdef HAVE_OPENMP
#pragma omp parallel default(shared)
#endif
      //std::cout << "Before while grid < number of subgrids Line 2029" << std::endl; // mgb comment 28.04.2026
      while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        const size_t this_igrid = igrid.post_increment();
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          MagnetoHydroDensitySubGrid &subgrid = *grid_creator->get_subgrid(this_igrid);
 ...
 // std::cout << "After first time step write out Line 2050" << std::endl; // mgb comment 28.04.2026
  /// RADIATION
  if (restart_reader == nullptr) {
    ...
    // parameter value (for now)
    {
      ...
      for (photonsourcenumber_t isource = 0; isource < number_of_sources;
        ...
        DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid >::iterator gridit =
  ...
  if (restart_reader == nullptr) {
    ...
    std::cout << "BEFORE ADVANCE -> Requested by CFL: " << requested_timestep << " s" << std::endl;
    
    has_next_step = timeline->advance(requested_timestep, actual_timestep, current_time);
    
    std::cout << "AFTER ADVANCE  -> Forced by Timeline: " << actual_timestep 
              << " s | Current Simulation Time: " << current_time << " s" << std::endl;
  } else {
 ...
 // double current_time_restarted = current_time + _restart_time; //current_time + _restart_time; // mgb edit 10.11.2025
  ...
  while (has_next_step && !stop_simulation) {
    ...
    if (log) {
      log->write_status("Starting magnetohydro step ", num_step, ", t = ",
  ...
  //  std::cout << "After Starting Hydro Step Line 2174" << std::endl; // mgb comment 28.04.2026
  // auto it = grid_creator->get_subgrid(0); // mgb edit 28.04.2026

   // MagnetoHydroDensitySubGrid* test_sg = &(*it); // mgb edit 28.04.2026

   // std::cout << "DEBUG: Subgrid 0 pointer address: " << test_sg << std::endl;


  //  std::cout << "DEBUG: Hydro variables address: " << test_sg->_hydro_variables << std::endl;
    // decide whether or not to do the radiation step
  //  std::cout << "Before radiation step Line 2184" << std::endl; // mgb comment 28.04.2026
    if (do_radiation &&
        (hydro_radtime < 0. ||
      ...
      //  std::cout << "After cooling file Line 2195" << std::endl; // mgb comment 28.04.2026
    ...
    //  std::cout << "Within source distribution file Line 2208" << std::endl; // mgb comment 28.04.2026
...
#pragma omp parallel default(shared)
          ...
          while (igrid.value() < grid_creator->number_of_original_subgrids()) {
            const size_t this_igrid = igrid.post_increment();
           // std::cout << "Within subgrids Line 2221" << std::endl; // mgb comment 28.04.2026

            if (this_igrid < grid_creator->number_of_original_subgrids()) {
              MagnetoHydroDensitySubGrid &subgrid =
                  *grid_creator->get_subgrid(this_igrid);
              //std::cout << "Got subgrid Line 2226" << std::endl; // mgb comment 28.04.2026
                ...
                //  std::cout << "After ionisation variables got Line 2236" << std::endl; // mgb comment 28.04.2026

                    
                }
              subgrid.update_ionization_variables(hydro,
                                                  maximum_neutral_fraction);
            //  std::cout << "After update ionisation vars Line 2242" << std::endl; // mgb comment 28.04.2026
       ...
       // std::cout << "DEBUG: Grid creator size: " << grid_creator->number_of_original_subgrids() << std::endl;// mgb
        const auto pos = sourcedistribution->get_position(0);
       // std::cout << "DEBUG: Source position: x=" << pos[0] << " y=" << pos[1] << " z=" << pos[2] << std::endl;

        DistributedPhotonSource< MagnetoHydroDensitySubGrid, DensitySubGridCreatorMHD<MagnetoHydroDensitySubGrid> > photon_source(
...
#pragma omp parallel default(shared)
#endif
            while (igrid.value() < grid_creator->number_of_actual_subgrids()) {
            //  std::cout << "Within grids Line 2292" << std::endl;
          ...
          //    std::cout << "Got subgrids Line 2299" << std::endl;
            }
            stop_parallel_timing_block();
          }

          // update copies
        //  std::cout << "before copy properties Line 2305" << std::endl;
          grid_creator->update_copy_properties();
        //  std::cout << "After copy properties Line 2307" << std::endl;
...
#pragma omp parallel default(shared)
         ...
         // std::cout << "After reemission handler Line 2331" << std::endl;
      ...
      //    std::cout << "After photon source and photons Line 2349" << std::endl;
         ...
         TaskContext *task_contexts[TASKTYPE_NUMBER] = {nullptr};
          task_contexts[TASKTYPE_SOURCE_DISCRETE_PHOTON] =
              new SourceDiscretePhotonTaskContext< MagnetoHydroDensitySubGrid, DensitySubGridCreatorMHD<MagnetoHydroDensitySubGrid >>(
          ...
          if (reemission_handler) {
            task_contexts[TASKTYPE_PHOTON_REEMIT] =
                new PhotonReemitTaskContext< MagnetoHydroDensitySubGrid, DensitySubGridCreatorMHD<MagnetoHydroDensitySubGrid> >(
          ...
          task_contexts[TASKTYPE_PHOTON_TRAVERSAL] =
              new PhotonTraversalTaskContext< MagnetoHydroDensitySubGrid, DensitySubGridCreatorMHD<MagnetoHydroDensitySubGrid> >(
                  *buffers, *grid_creator, *tasks, num_photon_done, nullptr,
                  reemission_handler != nullptr, _max_photon_distance);

          PrematureLaunchTaskContext< MagnetoHydroDensitySubGrid, DensitySubGridCreatorMHD<MagnetoHydroDensitySubGrid> > premature_launch(
...
#pragma omp parallel default(shared)
#endif
          {
           // std::cout << "Before threads initialisation Line 2388" << std::endl;
           ...
           // std::cout << "Within threads initialisation Line 2398" << std::endl;

            // actual run flag
            uint_fast32_t current_index = shared_queue->get_task(*tasks);
            while (global_run_flag) {
            //  std::cout << "Within threads global run Line 2403" << std::endl;

              if (current_index == NO_TASK) {
               // std::cout << "No task, trying premature launch Line 2406" << std::endl;
                premature_launch.execute();
                current_index = scheduler.get_task(thread_id);
              }

              while (current_index != NO_TASK) {
               // std::cout << "Within threads executing task Line 2411" << std::endl;
           ...
           // std::cout << "Deleting task contexts Line 2460" << std::endl;
        ...
        //  std::cout << "After tasks initialisation Line 2242" << std::endl;
          stop_parallel_timing_block();
...
#pragma omp parallel default(shared)
            ...
            while (igrid.value() <
              ...
              if (this_igrid < grid_creator->number_of_original_subgrids()) {
                ...
                if (_time_dependent_ionization) { // mgb note 21.05.2026 - time-dependent temp calcs 
                  if (iloop == nloop -1) {
...
//       }
  ...
  //  std::cout << "Before do explicit temp calc Line 2652" << std::endl; // mgb comment 28.04.2026

      {
  if (!do_explicit_temp_calc) { // mgb comment 21.05.2026: if not doing explicit_temp_calc this will happen
        time_logger.start("ionizing energy update");
        AtomicValue< size_t > igrid(0);
        start_parallel_timing_block();
#ifdef HAVE_OPENMP
#pragma omp parallel default(shared)
  ...
  //  std::cout << "Before MagnetoHydro Start Line 2675" << std::endl; // mgb comment 28.04.2026


    time_logger.start("magnetohydro");

    if (log) {
      log->write_status("Starting magnetohydro step...");
    }
...
#pragma omp parallel default(shared)
   ...
   // std::cout << "After external potential Line 2715" << std::endl; // mgb comment 28.04.2026
...
#pragma omp parallel default(shared)
      ...
      while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          for (auto it = subgrid.hydro_begin(); it != subgrid.hydro_end();
            ...
            IonizationVariables ionization_variables = it.get_ionization_variables(); // mgb edit 26.05.2026: for debug 
            MagnetoHydroVariables hydro_variables = it.get_hydro_variables();
            ...
            if (debug_flag == true){
                ...
                double mh = PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_PROTON_MASS);
                const double h0 = ionization_variables.get_ionic_fraction(ION_H_n);
                double inverse_volume = 1/it.get_volume();
              ...
              //  double AHe = 0.0;
                ...
                if (radiation_internal_energy > 100*hydro_internal_energy){
                  std::cout<< "Warning: radiation_internal_energy > 100*hydro_internal_energy after radiation! rad_Eint: " << radiation_internal_energy << ", hydro_Eint: " << hydro_internal_energy << std::endl;
                }
                if (hydro_internal_energy > 100*radiation_internal_energy){
                  std::cout<< "Warning: hydro_internal_energy > 100*radiation_internal_energy after radiation! rad_Eint: " << radiation_internal_energy << ", hydro_Eint: " << hydro_internal_energy << std::endl;
                }
   ...
   // std::cout << "After do self gravity Line 2778" << std::endl; // mgb comment 28.04.2026
...
#pragma omp parallel default(shared)
    ...
    // std::cout << "After turbulence forcing Line 2813" << std::endl; // mgb comment 28.04.2026

    // reset the hydro tasks and add them to the queue
    AtomicValue< uint_fast32_t > number_of_tasks;
    for (auto cellit = grid_creator->begin();
         cellit != grid_creator->original_end(); ++cellit) {
      // std::cout << "Within loop Line 2819, cellit = " << &(*cellit) << std::endl; // mgb comment 28.04.2026

      reset_hydro_tasks(*tasks, *cellit);
      // std::cout << "After reset hydro task call Line 2822" << std::endl; // mgb comment 28.04.2026

      for (int_fast8_t i = 0; i < 18; ++i) { // changed from 18 to 32
      //  std::cout << "Inside second loop Line 2825, i = " << (int)i << std::endl; // mgb comment 28.04.2026
        const size_t itask = (*cellit).get_hydro_task(i);
       // std::cout << "Got itask Line 2827, itask = " << itask << std::endl; // mgb comment 28.04.2026

        if (itask != NO_TASK &&
            (*tasks)[itask].get_number_of_unfinished_parents() == 0) {
       //   std::cout << "Adding task Line 2832, itask = " << itask << std::endl; // mgb comment 28.04.2026
          queues[(*cellit).get_owning_thread()]->add_task(itask);
          number_of_tasks.pre_increment();
        }
      }
    }
 //   std::cout << "After reset the hydro tasks Line 2829" << std::endl; // mgb comment 28.04.2026
...
#pragma omp parallel default(shared)
       ...
       while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          for (auto cellit = (*gridit).hydro_begin();
            ...
            MagnetoHydroVariables hydro_variables = cellit.get_hydro_variables();
  ...
  //  std::cout << "After first tasks Line 2875" << std::endl; // mgb comment 28.04.2026
...
#pragma omp parallel default(shared)
  ...
  //  std::cout << "After mask Line 2907" << std::endl; // mgb comment 28.04.2026


    if (sourcedistribution != nullptr) {
  //    std::cout << "If source 2911" << std::endl;
  ...
  //  std::cout << "before cpucycle Line 2922" << std::endl;
...
#pragma omp parallel default(shared)
      ...
      while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          for (auto cellit = (*gridit).hydro_begin();
               cellit != (*gridit).hydro_end(); ++cellit) {
           // std::cout << "Neutral Fraction before Hydro_to_ionization = " << cellit.get_ionization_variables().get_ionic_fraction(ION_H_n) << std::endl;
            
           // hydro.set_primitive_variables(cellit.get_hydro_variables(), cellit.get_ionization_variables(), cellit.get_volume())
            hydro.hydro_to_ionization(cellit.get_hydro_variables(), cellit.get_ionization_variables()); // mgb comment 22.05.2026: only sets the density 

            
            // std::cout << "Neutral Fraction after Hydro_to_ionization = " << cellit.get_ionization_variables().get_ionic_fraction(ION_H_n) << std::endl;
            ...
            if (cellit.get_hydro_variables().get_conserved_internal_energy() != hydro_variables.get_conserved_internal_energy()) {
              
              std::cout<<"Difference in Eint before any steps: " << hydro_variables.get_conserved_internal_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl;
            }  
            const bool debug_flag = true;
            if (debug_flag == true){
                double rho = hydro_variables.get_primitives_density();
              ...
              //  double AHe = 0.0;
                ...
                double hydro_internal_energy = hydro_variables.get_conserved_internal_energy();
                if (radiation_internal_energy > 100*hydro_internal_energy){
                  std::cout<< "Warning: radiation_internal_energy > 100*hydro_internal_energy in cooling block! rad_Eint: " << radiation_internal_energy << ", hydro_Eint: " << hydro_internal_energy << std::endl;
                }
                if (hydro_internal_energy > 100*radiation_internal_energy){
                  std::cout<< "Warning: hydro_internal_energy > 100*radiation_internal_energy in cooling block! rad_Eint: " << radiation_internal_energy << ", hydro_Eint: " << hydro_internal_energy << std::endl;
                }

            }
          
            //double temperature_before_cooling = cellit.get_ionization_variables().get_temperature();
            ...
            // }
              if (ionization_variables.get_ionic_fraction(ION_H_n) == 0) {
                cmac_warning("Zero neutral faction before do_explicit_heat_cool");
              }
              if (std::isnan(ionization_variables.get_temperature())) {
                cmac_warning("NaN temperature before do_explicit_heat_cool");
              }
              if (std::isnan(hydro_variables.get_conserved_total_energy())) {
                cmac_warning("NaN total energy before do_explicit_heat_cool");
              }
             // if (cellit.get_hydro_variables().get_conserved_internal_energy() != hydro_variables.get_conserved_internal_energy()) {
              
               // std::cout<<"Difference in Eint after do_explicit_temp_calc: " << hydro_variables.get_conserved_internal_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl;
             // }
              do_explicit_heat_cool(ionization_variables, hydro_variables,
                        1. / cellit.get_volume(), nH2 * cellit.get_volume(),
                        actual_timestep, radiative_cooling, hydro,
                          _cooling_temp_floor,_gamma-1., _mu0, line_cooling_data, abundances,
                          use_cool_tables); // mgb comment 21.05.2026 - does not update temperature but temp is already out of sync with internal energy before the energy update!
            }
            //if (cellit.get_hydro_variables().get_conserved_internal_energy() != hydro_variables.get_conserved_internal_energy()) {
          ...
          //  }
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within ionization update before energy update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
            }
            cellit.get_ionization_variables().set_temperature(
                ionization_variables.get_temperature()); // mgb comment 21.05.2026 - this just sets the temperature to the same as it was in do_explicit_heat_cool, temperature isn't actually updated??
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              
              std::cout<<"Warning Etot < Eint within ionization update after temperature update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
              std::cout<<"Hydro Variables set, Etot: " << hydro_variables.get_conserved_total_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_total_energy() << std::endl;
              std::cout<<"Hydro Variables set, Eint: " << hydro_variables.get_conserved_internal_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl;
            }  
            ...
            cellit.get_hydro_variables().set_conserved_internal_energy(
              hydro_variables.get_conserved_internal_energy());
            
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              
              std::cout<<"Warning Etot < Eint within ionization update after energy update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
              std::cout<<"Hydro Variables set, Etot: " << hydro_variables.get_conserved_total_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_total_energy() << std::endl;
              std::cout<<"Hydro Variables set, Eint: " << hydro_variables.get_conserved_internal_energy() << "cellit value: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl;
            }    
            if (ionization_variables.get_temperature() <
                _cooling_temp_floor) {
              if (ionization_variables.get_temperature() == 0){
                std::cout << "Warning: Temperature is zero after do_explicit_heat_cool! Temp: " << ionization_variables.get_temperature() << std::endl;
              }
                hydro.set_temperature(
                  cellit.get_ionization_variables(),
                  cellit.get_hydro_variables(), cellit.get_volume(),
                  _cooling_temp_floor);
               // hydro.align_p_to_temp(cellit.get_hydro_variables(), cellit.get_ionization_variables(), cellit.get_volume()); // mgb edit 26.05.2026: added function to try to align the P and Eint with T when its below the temperature floor - actually not needed as set_temperature in MagnetoHydro does this already

            }
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within ionization update before update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
            }
            hydro.set_conserved_variables(cellit.get_hydro_variables(), cellit.get_volume());

            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within ionization update after update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
            }
          /*
            double temperature_after_cooling = cellit.get_ionization_variables().get_temperature();

            if ((temperature_before_cooling > _temperature_to_trace) & (temperature_after_cooling < _temperature_to_trace)) {
              cellit.get_hydro_variables().set_conserved_cooled_gas_scalar_field(1.);
            }
            if ((temperature_before_cooling < _temperature_to_trace) & (temperature_after_cooling > _temperature_to_trace)) {
              cellit.get_hydro_variables().set_conserved_cooled_gas_scalar_field(0.0);
              cellit.get_hydro_variables().set_conserved_initial_cold_gas_scalar_field(0.0);
            }*/
   ...
   // }
    ...
    if (write_output && hydro_lastsnap * hydro_snaptime <= current_time-actual_timestep &&
      ...
      if (hydro_firstsnap <= hydro_lastsnap) {
        ...
        //  std::cout << "before write Line 3030" << std::endl;
          writer->write(*grid_creator, hydro_lastsnap_restart, *params, current_time_restarted);
        //  std::cout << "after write Line 3032" << std::endl;
          time_logger.end("snapshot");
        } else {
          time_logger.start("snapshot");
        //  std::cout << "before write Line 3036" << std::endl;
          writer->write(*grid_creator, hydro_lastsnap, *params, current_time);
        //  std::cout << "after write Line 3038" << std::endl;
          time_logger.end("snapshot");
        }
      }
      ++hydro_lastsnap;
...
#pragma omp parallel default(shared)
    ...
    if (do_stellar_feedback &&
        sourcedistribution->do_stellar_feedback(current_time)) { // gets number of supernovae
       if (log) {
...
#pragma omp parallel default(shared)
      ...
      while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          sourcedistribution->add_stellar_feedback(subgrid, hydro); // adds momentum if unresolved and updates energy term
          ...
          for (auto cellit = (*gridit).hydro_begin();
            ...
            if (dE > 0.0) { // if resolved this should be greater than zero.
             // if (std::abs(dE) > cellit.get_hydro_variables().get_conserved_internal_energy()){
             //   std::cout << "Warning: supernova, Eint < dE: " << cellit.get_hydro_variables().get_conserved_internal_energy() << ", dE: " << dE << std::endl;
             // }
              double energy_before = cellit.get_hydro_variables().get_conserved_total_energy(); 
              hydro.update_energy_variables(cellit.get_ionization_variables(),
                                            cellit.get_hydro_variables(),
            ...
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within stellar feedback 2 loop after update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
            }
            hydro.set_conserved_variables(cellit.get_hydro_variables(), cellit.get_volume());
            if (cellit.get_hydro_variables().get_conserved_total_energy() < cellit.get_hydro_variables().get_conserved_internal_energy()) {
              std::cout<<"Warning Etot < Eint within stellar feedback loop 2 after update! Etot: " << cellit.get_hydro_variables().get_conserved_total_energy() << ", Eint: " << cellit.get_hydro_variables().get_conserved_internal_energy() << std::endl; 
            }
          //  if (cellit.get_ionization_variables().get_temperature() > 9.9999e8) {
          //    hydro.set_temperature(cellit.get_ionization_variables(), cellit.get_hydro_variables(),
...
#pragma omp parallel default(shared)
    ...
    if (restart_manager.write_restart_file() || stop_simulation) {
      ...
      if (hydro_mask != nullptr) {
        MagnetoHydroMaskFactory::write_restart_file(*restart_writer, *hydro_mask);
      }
