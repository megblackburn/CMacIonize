 ...
 */
class HomogeneousDensityFunctionMHD : public DensityFunctionMHD {
private:
...
public:
   ...
   */
  HomogeneousDensityFunctionMHD(double density = 1., double temperature = 8000., 
                            const double init_Bx = 0.,
                            const double init_By = 0.,
                            const double init_Bz = 0.,
                            const double init_B_scalar = 0.,
                             double neutral_fraction_H = 1.e-6,
                             double dust_to_gas = 0.0, double fraction_silicates = 0.6,
                             Log *log = nullptr)
      : _density(density), _temperature(temperature), _init_Bx(init_Bx), _init_By(init_By), _init_Bz(init_Bz), _init_B_scalar(init_B_scalar),
        _neutral_fraction_H(neutral_fraction_H),
        _dust_to_gas(dust_to_gas),_fraction_silicates(fraction_silicates) {

    if (log) {
      log->write_status(
          "Created HomogeneousDensityFunctionMHD with constant density ", _density,
   ...
   * @param log Log to write logging information to.
   */
  HomogeneousDensityFunctionMHD(ParameterFile &params, Log *log = nullptr)
      : HomogeneousDensityFunctionMHD(
  ...
  virtual DensityValues operator()(const Cell &cell) {
   ...
   // std::cout << "Initialized in HomogeneousDensityFunctionMHD 179 " << std::endl;
    return values;
  }
};
