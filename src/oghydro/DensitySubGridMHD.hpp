 ...
 */
#ifndef DENSITYSUBGRIDMHD_HPP
#define DENSITYSUBGRIDMHD_HPP
...
#define DENSITYSUBGRID_MHD_FIXED_MPI_SIZE                                          \
 ...
 *  time. */
#define DENSITYSUBGRID_MHD_FIXED_SIZE sizeof(DensitySubGridMHD)

/*! @brief Size of a single cell of the subgrid. */
#define DENSITYSUBGRID_MHD_ELEMENT_SIZE sizeof(IonizationVariables)
...
protected:
  ...
  MagnetoHydroVariables *_hydro_variables; // mgb edit 28.04.2026 - pointer to the array of hydrodynamical variables for each cell
  /**
...
public:
   ...
   */
  inline DensitySubGridMHD(const double *box,
   ...
   */
  inline DensitySubGridMHD(const DensitySubGridMHD &original)
  ...
  inline int_fast32_t get_MPI_size() const {
    return DENSITYSUBGRID_MHD_FIXED_MPI_SIZE + DENSITYSUBGRID_MHD_ELEMENT_SIZE *
                                               _number_of_cells[0] *
                                               _number_of_cells[3];
  }
  ...
  inline size_t get_memory_size() const {
    return DENSITYSUBGRID_MHD_FIXED_SIZE + DENSITYSUBGRID_MHD_ELEMENT_SIZE *
                                           _number_of_cells[0] *
                                           _number_of_cells[3];
  }
   ...
   */
  inline void update_neutral_fractions(const DensitySubGridMHD &original) {
    const int_fast32_t tot_ncell = _number_of_cells[3] * _number_of_cells[0];
    for (int_fast32_t i = 0; i < tot_ncell; ++i) {
   ...
   */
  inline void update_intensities(const DensitySubGridMHD &copy) {
   ...
   */
  inline void check_equal(const DensitySubGridMHD &other) {
  ...
  public:
     ...
     */
    inline iterator(const uint_fast32_t index, DensitySubGridMHD &subgrid)
    ...
    inline const MagnetoHydroVariables &get_hydro_variables() const {
      cmac_error("A DensitySubGrid has no magnetohydro variables!");
      return reinterpret_cast< MagnetoHydroVariables & >(
   ...
   */
  inline DensitySubGridMHD(RestartReader &restart_reader) {
