...
/**
 * @file BlockSyntaxMagnetoHydroMask.hpp
 *
 ...
 */
#ifndef BLOCKSYNTAXMAGNETOHYDROMASK_HPP
#define BLOCKSYNTAXMAGNETOHYDROMASK_HPP

#include "BlockSyntaxDensityFunctionMHD.hpp"
 ...
 * constant value.
 */
class BlockSyntaxMagnetoHydroMask : public MagnetoHydroMask {
private:
  /*! @brief BlockSyntaxDensityFunction that contains the masked out blocks. */
  BlockSyntaxDensityFunctionMHD _density_function;
...
public:
   ...
   */
  inline BlockSyntaxMagnetoHydroMask(std::string filename, const double gamma,
      ...
      : _density_function(filename), _gm1inv(1. / (gamma - 1.)),
        _vprof_velocity(std::sqrt(PhysicalConstants::get_physical_constant(
                                  ...
                                  vprof_mass)), _mu0(PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_MAGNETC_PERMEABILITY)) {}
   ...
   */
  inline BlockSyntaxMagnetoHydroMask(ParameterFile &params)
      : BlockSyntaxMagnetoHydroMask(
            params.get_value< std::string >("MagnetoHydroMask:filename"),
            params.get_value< double >("MagnetoHydroMask:polytropic index", 5. / 3.),
            params.get_physical_value< QUANTITY_MASS >("MagnetoHydroMask:vprof mass",
                                                       "0. kg")) {}
  /**
   * @brief Virtual destructor.
   */
  virtual ~BlockSyntaxMagnetoHydroMask() {}
  ...
  virtual void apply_mask(DensityGridMHD &grid, const double actual_timestep,
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      ...
      if (_density_function.inside(position)) {
        ...
        const double total_energy = volume * pressure * _gm1inv + 0.5 * ekin + emag;
...
#endif // BLOCKSYNTAXMAGNETOHYDROMASK_HPP
