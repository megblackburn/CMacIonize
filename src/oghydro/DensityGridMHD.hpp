 ...
 */
#ifndef DENSITYGRIDMHD_HPP
#define DENSITYGRIDMHD_HPP
...
protected:
  ...
  std::vector< MagnetoHydroVariables > _hydro_variables;
   ...
   * @param photon Photon.
   */
  inline void update_integrals(double ds, DensityGridMHD::iterator &cell,
...
public:
   ...
   */
  virtual DensityGridMHD::iterator interact(Photon &photon,
  ...
  public:
     ...
     */
    inline iterator(cellsize_t index, DensityGridMHD &grid)
  ...
  public:
     ...
     */
    DensityGridInitializationFunction(DensityFunctionMHD &function, bool hydro)
  ...
  void set_densities(std::pair< cellsize_t, cellsize_t > &block,
                     DensityFunctionMHD &function, int_fast32_t worksize = -1);
   ...
   * created cells.
   */
  virtual void reset_grid(DensityFunctionMHD &density_function) {
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    }
    {
      ...
      restart_writer.write(size);
      for (std::vector< MagnetoHydroVariables >::size_type i = 0; i < size; ++i) {
   ...
   */
  inline DensityGridMHD(RestartReader &restart_reader, Log *log = nullptr)
    ...
    {
      const std::vector< MagnetoHydroVariables >::size_type size =
          restart_reader.read< std::vector< MagnetoHydroVariables >::size_type >();
