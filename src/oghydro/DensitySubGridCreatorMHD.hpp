 ...
 */
#ifndef DENSITYSUBGRIDCREATORMHD_HPP
#define DENSITYSUBGRIDCREATORMHD_HPP
...
public:
   ...
   */
  inline DensitySubGridCreatorMHD(
   ...
   */
  inline DensitySubGridCreatorMHD(const Box<> box, ParameterFile &params)
      : DensitySubGridCreatorMHD(
            box,
   ...
   * variables.
   */
  inline void initialize(DensityFunctionMHD &density_function) {
...
#pragma omp parallel default(shared)
  ...
  private:
    ...
    DensitySubGridCreatorMHD *_grid_creator;
  ...
  public:
    ...
    inline iterator(const uint_fast32_t index,
                    DensitySubGridCreatorMHD &grid_creator)
...
std::vector<std::pair<uint_fast32_t, uint_fast32_t>> cells_within_radius(CoordinateVector<double> midpoint, double radius) {
        ...
        for (int y_idx = min_idx.y(); y_idx <= max_idx.y(); ++y_idx) {
            for (int z_idx = min_idx.z(); z_idx <= max_idx.z(); ++z_idx) {
                ...
                if (distance <= radius) {
                    ...
                    _subgrid_type_& subgrid = *this->get_subgrid(subgrid_idx);
   ...
   */
  inline DensitySubGridCreatorMHD(RestartReader &restart_reader)
