 ...
 */
#include "GadgetDensityGridWriterMHD.hpp"
#include "Box.hpp"
 ...
 */
GadgetDensityGridWriterMHD::GadgetDensityGridWriterMHD(
    std::string prefix, std::string output_folder, const bool hydro,
    const DensityGridWriterFieldsMHD fields, Log *log, uint_fast8_t padding,
    const bool compression)
    : DensityGridWriterMHD(output_folder, hydro, fields, log), _prefix(prefix),
 ...
 * @param log Log to write logging information to.
 */
GadgetDensityGridWriterMHD::GadgetDensityGridWriterMHD(std::string output_folder,
                                                 ParameterFile &params,
                                                 const bool hydro, Log *log)
    : GadgetDensityGridWriterMHD(
          params.get_value< std::string >("DensityGridWriter:prefix",
                                          "snapshot"),
          output_folder, hydro, DensityGridWriterFieldsMHD(params, hydro), log,
 ...
 */
void GadgetDensityGridWriterMHD::write(DensityGridMHD &grid, uint_fast32_t iteration,
                                    ParameterFile &params, double time,
                                    const InternalMagnetoHydroUnits *hydro_units) {
...
//  const bool _restart_flag = params.get_value< bool >("DensityGridWriter:restart flag", false);
 ...
 // }
  ...
  double unit_magnetic_field_in_cgs = 1.; // mgb edit 01.05.2026
  ...
  HDF5Tools::write_attribute< double >(group, "Unit time in cgs (U_t)",
                                       unit_time_in_cgs);
  HDF5Tools::write_attribute< double >(group, "Unit magnetic field in cgs (U_B)", // mgb edit 01.05.2026
                                       unit_magnetic_field_in_cgs);
  HDF5Tools::close_group(group);
  ...
  for (int_fast32_t property = 0; property < DENSITYGRIDFIELDMHD_NUMBER;
    ...
    if (_fields.field_present(property)) {
      const std::string name = DensityGridWriterFieldsMHD::get_name(property);
      if (DensityGridWriterFieldsMHD::get_type(property) ==
          DENSITYGRIDFIELDTYPEMHD_VECTOR_DOUBLE) {
        HDF5Tools::create_dataset< CoordinateVector<> >(group, name, numpart[0],
                                                        _compression);
      } else {
        if (DensityGridWriterFieldsMHD::is_ion_property(property)) {
        ...
        } else if (DensityGridWriterFieldsMHD::is_heating_property(property)) {
  ...
  for (uint_fast32_t iblock = 0; iblock < numblock; ++iblock) {
    ...
    std::vector< std::vector< CoordinateVector<> > > vector_props(
        _fields.get_field_count(DENSITYGRIDFIELDTYPEMHD_VECTOR_DOUBLE),
        std::vector< CoordinateVector<> >(thisblocksize));
    std::vector< std::vector< double > > scalar_props(
        _fields.get_field_count(DENSITYGRIDFIELDTYPEMHD_SCALAR_DOUBLE),
        ...
        if (_fields.field_present(property)) {
          if (DensityGridWriterFieldsMHD::get_type(property) ==
            ...
            vector_props[vector_index][index] =
                DensityGridWriterFieldsMHD::get_vector_double_value(
            ...
            if (DensityGridWriterFieldsMHD::is_ion_property(property)) {
              for (int_fast32_t ion = 0; ion < NUMBER_OF_IONNAMES; ++ion) {
                if (_fields.ion_present(property, ion)) {
                  scalar_props[scalar_index][index] =
                      DensityGridWriterFieldsMHD::get_scalar_double_ion_value(
                          property, ion, it);
                  ++scalar_index;
                }
              }
              ...
              for (int_fast32_t heating = 0; heating < NUMBER_OF_HEATINGTERMS;
                ...
                if (_fields.heatingterm_present(property, heating)) {
                  scalar_props[scalar_index][index] =
                      DensityGridWriterFieldsMHD::get_scalar_double_heating_value(
                          property, heating, it);
                  ++scalar_index;
                }
              }
              ...
              scalar_props[scalar_index][index] =
                  DensityGridWriterFieldsMHD::get_scalar_double_value(property, it,
                                                                   hydro_units);
              ++scalar_index;
            }
          }
 ...
 * @param time Simulation time (in s).
 */
void GadgetDensityGridWriterMHD::write(
    DensitySubGridCreatorMHD< DensitySubGridMHD > &grid_creator,
  ...
  const DensityGridWriterFieldsMHD &fields = _fields;
  ...
  for (auto gridit = grid_creator.begin();
    ...
    for (uint_fast32_t iblock = 0; iblock < numblock; ++iblock) {
      ...
      std::vector< std::vector< CoordinateVector<> > > vector_props(
          fields.get_field_count(DENSITYGRIDFIELDTYPEMHD_VECTOR_DOUBLE),
          std::vector< CoordinateVector<> >(thisblocksize));
      std::vector< std::vector< double > > scalar_props(
          fields.get_field_count(DENSITYGRIDFIELDTYPEMHD_SCALAR_DOUBLE),
          std::vector< double >(thisblocksize));
        ...
        for (int_fast32_t property = 0; property < DENSITYGRIDFIELDMHD_NUMBER;
          ...
          if (fields.field_present(property)) {
            if (DensityGridWriterFieldsMHD::get_type(property) ==
              ...
              if (DensityGridWriterFieldsMHD::is_ion_property(property)) {
              ...
              } else if (DensityGridWriterFieldsMHD::is_heating_property(
                             property)) {
                for (int_fast32_t heating = 0; heating < NUMBER_OF_HEATINGTERMS;
                     ++heating) {
                  if (fields.heatingterm_present(property, heating)) {
                    scalar_props[scalar_index][index] =
                        DensityGridWriterFieldsMHD::
                ...
                scalar_props[scalar_index][index] =
                    DensityGridWriterFieldsMHD::get_scalar_double_value(property,
                                                                     cellit);
                ++scalar_index;
              }
            }
