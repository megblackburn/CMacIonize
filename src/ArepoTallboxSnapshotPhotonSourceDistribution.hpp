...
/**
 * @file ArepoTallboxSnapshotPhotonSourceDistribution.hpp
 *
 * @brief Bursty PhotonSourceDistribution.
 *
 * @author Meg Blackburn (mgb27@st-andrews.ac.uk)
 */
#ifndef AREPOTALLBOXSNAPSHOTPHOTONSOURCEDISTRIBUTION_HPP
#define AREPOTALLBOXSNAPSHOTPHOTONSOURCEDISTRIBUTION_HPP
 ...
 */
class ArepoTallboxSnapshotPhotonSourceDistribution : public PhotonSourceDistribution {
private:
...
// const double bursty_star_formation_rate; 
  ...
  const bool _add_arepo_sources; // mgb edit 19.06.2026
  /*! flag for reading in arepo sources from csv */

  const bool _use_mw_sfh; // mgb edit 19.06.2026
 /*! use the MW SFH profile */
  const double _start_mw_sfh_time;
 /*! what time to start the profile */
...
// std::vector< CoordinateVector<> > _source_positions_moving; // mgb edit from SinkSource
  ...
  std::string _arepo_source_filename;
  double _time;
...
public:
   ...
   */
  inline ArepoTallboxSnapshotPhotonSourceDistribution(
      ...
      const bool add_arepo_sources, // mgb edit 19.06.2026
      const bool use_mw_sfh, // mgb edit 19.06.2026
      const double start_mw_sfh_time, // mgb edit 19.06.2026
    //  const bool moving_sources_flag, // mgb 16.10.2025
      ...
      const std::string arepo_source_filename = "ArepoSnapshot_2x2x12_7.8pc_V5_490_sources.csv",
      ...
      : _star_formation_rate(star_formation_rate), _bursty_sfr_flag(bursty_sfr_flag), _KS_scaling_flag(KS_scaling_flag), _length_of_burst(length_of_burst), _time_of_burst_peak(time_of_burst_peak),
        ...
        _M_init(M_init), _M_init_unscaled_for_sfr(M_init_unscaled_for_sfr), _add_arepo_sources(add_arepo_sources), _use_mw_sfh(use_mw_sfh), _start_mw_sfh_time(start_mw_sfh_time),
        ...
        _read_file(read_file), _filename(filename), _source_filename(source_filename), _total_luminosity_filename(total_luminosity_filename), _sfr_filename(sfr_filename), _arepo_source_filename(arepo_source_filename), _time(time),
    ...
    if (output_sources) {
        ...
        *_output_file2 << "simulation time (s)\tlum (s^-1)\tnumsne\tSFR_base (Msol Myr-1)\tSFR_KS (Msol Myr^-1 kpc^-2)\tSFR_KS (kg s^-1 kpc^-2)\tM_init\tM_init_unscaled_for_sfr\n";
        _output_file2->flush();
  ...
  if (_add_arepo_sources){
      std::ifstream file;
      file.open(_arepo_source_filename);
      if (!file.is_open()) {
        cmac_error("Could not open Arepo Sources file \"%s\"!", _arepo_source_filename.c_str());
      } else {
        std::cout << "Opened Arepo Sources file - " << _arepo_source_filename << std::endl;
      }
      ...
      std::getline(file,dline);
        ...
        double val_mass, val_x, val_y, val_z;

        //while (!file.eof()) {
        file >> val_mass >> val_x >> val_y >> val_z;
            

            _source_positions.push_back(CoordinateVector<double>(val_x,val_y,val_z));
            _source_masses.push_back(val_mass);
            _source_luminosities.push_back(lum_from_mass(val_mass));
            double interpolatedTemp = interpolate(val_mass, stellarMasses, temperatures);
            size_t closestIndex = findClosestIndex(interpolatedTemp, avail_temps);
            _spectrum_index.push_back(closestIndex);
          ...
          double lifetime = a0z + a1z*std::log10(val_mass) + a2z*(std::log10(val_mass)*std::log10(val_mass));
          lifetime = std::pow(10.0,lifetime);
          lifetime = lifetime*3.154e+7;
            ...
            _source_indices.push_back(_next_index);
             ...
             std::cout << "Adding CSV star of mass (Msun): " << val_mass 
                      << " | Temp: " << interpolatedTemp 
                      << " | Spec Index: " << closestIndex << std::endl;
   ...
   */
  ArepoTallboxSnapshotPhotonSourceDistribution(ParameterFile &params, Log *log = nullptr)
      : ArepoTallboxSnapshotPhotonSourceDistribution(
            ...
            params.get_value< bool >("PhotonSourceDistribution:add arepo sources", true), // mgb edit 30.06.2026
            params.get_value< bool >("PhotonSourceDistribution:use mw sfh", true), // mgb edit 19.06.2026
            params.get_physical_value< QUANTITY_TIME >("PhotonSourceDistribution:start mw sfh time", "0.0 Myr"), 
         ...
         //   params.get_value< bool >("PhotonSourceDistribution: moving sources flag", false),
            ...
            params.get_value<std::string>("PhotonSourceDistribution:arepo source filename", "ArepoSnapshot_1x1x6_7.8pc_V5_490.hdf5"),
   ...
   */
  virtual ~ArepoTallboxSnapshotPhotonSourceDistribution() {}
  ...
  virtual void add_stellar_feedback(MagnetoHydroDensitySubGrid &subgrid, MagnetoHydro &mhd) {

    for (uint_fast32_t i = 0; i < _to_do_feedback.size(); ++i) {

      novahandler->inject_sne(subgrid, mhd, _to_do_feedback[i], _r_inj[i],_r_st[i],_nbar[i],_num_cells_injected[i]);
    }
  }
  ...
  //  CoordinateVector<> cell_vel = cell.get_hydro_variables().get_primitives_velocity();
    ...
    //while (i < _source_lifetimes.size()) {
      std::cout << "i = " << i << std::endl;
      //float sources
      MagnetoHydroDensitySubGrid subgrid = *grid_creator->get_subgrid(_source_positions[i]);
      std::cout << "subgrid defined" << std::endl;
      auto cell = subgrid.get_hydro_cell(_source_positions[i]);
      std::cout << "cell defined" << std::endl;
      CoordinateVector<> cell_vel = cell.get_hydro_variables().get_primitives_velocity();
...
//      std::cout << "Source position after = " << _source_positions[i] << std::endl;
  ...
  // Type II supernovae: ///
    ...
    // this is where we want to add stellarwinds(mass, temp, lum) // mgb note 11.02.26
    size_t i = 0;
    while (i < _source_lifetimes.size()) {
...
// Stellar Sources
    ...
    //  double star_formation_rate = 0.;
      mwsfr = new ReadMilkyWaySFR(_sfr_filename);
      double sfr = _star_formation_rate;

      if (_use_mw_sfh == true){ // mgb comment: if true, uses a derived SFH
        if (_total_time < _start_mw_sfh_time) { // if before initialisation, sets the star formation rate to the first SFR - artificially extends the SFH
           sfr = mwsfr->get_sfr_at_time(0.0);
        } else {
          double sfh_time = _total_time - _start_mw_sfh_time;
           sfr = mwsfr->get_sfr_at_time(sfh_time);
        }
      }
      ...
      if (_KS_scaling_flag == true){
         mass_to_generate = _update_interval*sfr/unit_Msol*0.207*(std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index)); // replaced 1.988e30 with unit_Msol mgb edit 12.11.2025 
      } else {
         mass_to_generate = _update_interval*sfr/unit_Msol*0.207; // mgb edit 25.05.2026
      }

      if (_output_file2 != nullptr) {
        double totallum = get_total_luminosity();
        *_output_file2 << _total_time << "\t" << totallum << "\t" << _num_sne << "\t" << ((sfr*unit_Myr)/(unit_Msol)) << "\t" << ((sfr*unit_Myr)/(area_kpc*unit_Msol))*std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index) << "\t" <<  ((sfr/area_kpc)*std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index)) << "\t" << init_running_mass << "\t" << init_running_mass_unscaled_for_sfr << "\n"; // output the SFR in Msol Myr^-1 kpc^-2 and in kg s^-1 kpc^-2
        _output_file2->flush();

      }

       std::cout << "SHOULD BE GENERATING " << mass_to_generate - _excess_mass<< std::endl;
       std::cout << " _update_interval: " << _update_interval << ", SFR: " << sfr << ", mass_to_generate: " << mass_to_generate << std::endl;
   ...
   * @return True if the distribution changed, false otherwise.
   */
   virtual bool update(DensitySubGridCreatorMHD< MagnetoHydroDensitySubGrid > *grid_creator, double actual_timestep) override {
...
// Stellar Sources
    if (_total_time - _last_sf > _update_interval) {
      ...
      if (_use_mw_sfh == true){ // mgb comment: if true, uses a derived SFH
        if (_total_time < _start_mw_sfh_time) { // if before initialisation, sets the star formation rate to the first SFR - artificially extends the SFH
           star_formation_rate = mwsfr->get_sfr_at_time(0.0);
        } else {
          double sfh_time = _total_time - _start_mw_sfh_time;
           star_formation_rate = mwsfr->get_sfr_at_time(sfh_time);
        }
      } else {
         star_formation_rate = _star_formation_rate;
      }
      ...
      double mass_to_generate = 0.;
      if (_KS_scaling_flag == true){
         mass_to_generate = _update_interval*_star_formation_rate/unit_Msol*0.207*(std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index)); // replaced 1.988e30 with unit_Msol mgb edit 12.11.2025 
      } else {
         mass_to_generate = _update_interval*_star_formation_rate/unit_Msol*0.207; // mgb edit 25.05.2026
      }

      if (_output_file2 != nullptr) {
        double totallum = get_total_luminosity();
        *_output_file2 << _total_time << "\t" << "\t" << totallum << "\t" << _num_sne << "\t" << ((_star_formation_rate*unit_Myr)/(unit_Msol)) << "\t" << ((_star_formation_rate*unit_Myr)/(area_kpc*unit_Msol))*std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index) << "\t" <<  ((_star_formation_rate/area_kpc)*std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index)) << "\t" << init_running_mass << "\t" << init_running_mass_unscaled_for_sfr << "\n"; // output the SFR in Msol Myr^-1 kpc^-2 and in kg s^-1 kpc^-2
        _output_file2->flush();
      }
...
// --------------------------------------
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    restart_writer.write(_add_arepo_sources); // mgb edit 30.06.2026
    restart_writer.write(_use_mw_sfh); // mgb edit 30.06.2026
    restart_writer.write(_start_mw_sfh_time); // mgb edit 30.06.2026
   ...
   */
  inline ArepoTallboxSnapshotPhotonSourceDistribution(RestartReader &restart_reader)
      ...
      //  _moving_sources_flag(restart_reader.read< bool >())
        ...
        _add_arepo_sources(restart_reader.read< bool >()), // mgb edit 30.06.2026
        _use_mw_sfh(restart_reader.read< bool >()), // mgb edit 30.06.2026
        _start_mw_sfh_time(restart_reader.read< double>()), // mgb edit 30.06.2026
...
#endif // AREPOTALLBOXSNAPSHOTPHOTONSOURCEDISTRIBUTION_HPP
