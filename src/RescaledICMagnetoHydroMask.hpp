...
/**
 * @file RescaledICMagnetoHydroMask.hpp
 *
 * @brief Masked out region where the magneto-hydrodynamics is artificially reset to a
 * constant value.
 *
 * @author Meg Blackburn (mgb26@st-andrews.ac.uk)
 */
#ifndef RESCALEDICMAGNETOHYDROMASK_HPP
#define RESCALEDICMAGNETOHYDROMASK_HPP
 ...
 * constant value.
 */
class RescaledICMagnetoHydroMask : public MagnetoHydroMask {
private:
  ...
  /*! @brief Magnetic permeability (in kg m^3 s^-2 A^-2). */
  double _mu0;
...
public:
   ...
   */
  inline RescaledICMagnetoHydroMask(const CoordinateVector<> center,
                             const double radius,
      ...
      : _center(center),
        ...
        _mask_velocity(DBL_MAX), _mask_pressure(DBL_MAX), _mu0(PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_MAGNETC_PERMEABILITY)) {}
   ...
   */
  inline RescaledICMagnetoHydroMask(ParameterFile &params)
      : RescaledICMagnetoHydroMask(
            params.get_physical_vector< QUANTITY_LENGTH >("MagnetoHydroMask:center",
                                                          "[0. m, 0. m, 0. m]"),
            params.get_physical_value< QUANTITY_LENGTH >("MagnetoHydroMask:radius",
                                                         "1. m"),
            params.get_value< double >("MagnetoHydroMask:scale factor density", 0.01),
            params.get_value< double >("MagnetoHydroMask:scale factor velocity", 1.),
            params.get_value< double >("MagnetoHydroMask:scale factor pressure", 0.01),
            params.get_physical_value< QUANTITY_TIME >("MagnetoHydroMask:delta t",
                                                       "5000. yr")) {}

  /**
   * @brief Virtual destructor.
   */
  virtual ~RescaledICMagnetoHydroMask() {}
   ...
   */
  virtual void initialize_mask(DensityGridMHD &grid) {
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      if (is_inside(it.get_cell_midpoint())) {
        const MagnetoHydroVariables &hydro_variables = it.get_hydro_variables();
   ...
   * @param subgrid MagnetoHydroDensitySubGrid to read from.
   */
   ...
   */
  virtual void apply_mask(DensityGridMHD &grid, const double actual_timestep,
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      ...
      if (is_inside(it.get_cell_midpoint())) {
        ...
        const double emag = 0.5 * CoordinateVector<>::dot_product(it.get_hydro_variables().get_primitives_magnetic_field(), it.get_hydro_variables().get_primitives_magnetic_field()) * volume / _mu0; // mgb edit 21.04.2026 - calculate magnetic energy from primitive magnetic field strength
        // E = V*(rho*u + 0.5*rho*v^2) = V*(P/(gamma-1) + 0.5*rho*v^2)
        const double total_energy = A * pressure + 0.5 * ekin + emag;
   ...
   */
  inline RescaledICMagnetoHydroMask(RestartReader &restart_reader)
...
#endif // RESCALEDICMAGNETOHYDROMASK_HPP
