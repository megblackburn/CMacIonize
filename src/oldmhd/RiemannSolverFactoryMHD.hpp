...
#include "RiemannSolverMHD.hpp"

// implementations
#include "ExactRiemannSolver.hpp"
#include "HLLDRiemannSolver.hpp" // mgb edit 21.04.2026 - add HLLD Riemann solver for MHD
...
public:
  ...
  static RiemannSolver *generate(const std::string type = "Exact",
                                 const double gamma = 2., const double max_vel = 1.e99, const double cleaning_factor = 1.0) : 
                                  {

    if (type == "HLLD") {
      return new HLLDRiemannSolver(gamma, max_vel, cleaning_factor); // mgb edit 21.04.2026 - add mu0 and cleaning speed parameters for MHD
    } else {
