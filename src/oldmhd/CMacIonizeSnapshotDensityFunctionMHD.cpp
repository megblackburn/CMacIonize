 ...
 */
CMacIonizeSnapshotDensityFunctionMHD::CMacIonizeSnapshotDensityFunctionMHD(
...
CMacIonizeSnapshotDensityFunctionMHD::CMacIonizeSnapshotDensityFunctionMHD(
    ...
    : CMacIonizeSnapshotDensityFunctionMHD(
 ...
 */
void CMacIonizeSnapshotDensityFunctionMHD::initialize() {
  ...
  if (_log) {
    _log->write_info("Magnetic Field...");
  }

  // velocities (if they exist)
  std::vector< CoordinateVector<> > cell_magnetic_field;
  if (HDF5Tools::group_exists(group, "MagneticField")) {
    cell_magnetic_field =
        HDF5Tools::read_dataset< CoordinateVector<> >(group, "MagneticField");
  }

  if (_log) {
    _log->write_info("Magnetic Field Scalar...");
  }

  // velocities (if they exist)
  std::vector< double > cell_magnetic_field_scalar;
  if (HDF5Tools::group_exists(group, "B_Scalar")) {
    cell_magnetic_field_scalar =
        HDF5Tools::read_dataset< double >(group, "B_Scalar");
  }
...
// mgb edit start 05.03.2026: shuffle the velocities of cells with temperature < 1000 K to avoid forming stars in already ionised regions 
  ...
  if (type == "Cartesian") {
    ...
    for (size_t i = 0; i < cell_midpoints.size(); ++i) {
      ...
      if (cell_magnetic_field.size() > 0){
        _cartesian_grid[ix][iy][iz].set_magnetic_field(cell_magnetic_field[i]);
      }
      _cartesian_grid[ix][iy][iz].set_B_scalar(cell_magnetic_field_scalar[i]);
    }
    ...
    _log->write_status(
        "Constructing a Cartesian Task-Based Grid");
    _cartesian_grid = new DensityValues **[_ncell.x()];
    for (uint_fast32_t ix = 0; ix < _ncell.x(); ++ix) {
      _cartesian_grid[ix] = new DensityValues *[_ncell.y()];
      for (uint_fast32_t iy = 0; iy < _ncell.y(); ++iy) {
          ...
          for (uint_fast32_t cix = 0; cix < numblockx; ++cix) {
            for (uint_fast32_t ciy = 0; ciy < numblocky; ++ciy) {
              for (uint_fast32_t ciz = 0; ciz < numblockz; ++ciz) {
                ...
                if (cell_magnetic_field_scalar.size()>0){
                  _cartesian_grid[ix][iy][iz].set_B_scalar(cell_magnetic_field_scalar[cell_index]);
                }
               // _log->write_status("Magnetic field scalar set!");
                for (int_fast32_t ion = 0; ion < NUMBER_OF_IONNAMES; ++ion) {
                 _cartesian_grid[ix][iy][iz].set_ionic_fraction(
                ...
                if (cell_magnetic_field.size() > 0) { // mgb edit 26.05.2026
                  _cartesian_grid[ix][iy][iz].set_magnetic_field(
                      cell_magnetic_field[cell_index]);
                }
               // _log->write_status("Magnetic field set!");
    ...
    for (size_t i = 0; i < cell_midpoints.size(); ++i) {
      ...
      values.set_B_scalar(cell_magnetic_field_scalar[i]);
      for (int_fast32_t ion = 0; ion < NUMBER_OF_IONNAMES; ++ion) {
      ...
      if (cell_magnetic_field.size() > 0) {
        values.set_magnetic_field(cell_magnetic_field[i]);
      }
    }
  } else if (type == "Voronoi") {
    _log->write_status(
        "Constructing a Voronoi Grid");
    ...
    for (size_t i = 0; i < cell_midpoints.size(); ++i) {
      ...
      _voronoi_densityvalues[i].set_B_scalar(cell_magnetic_field_scalar[i]);
      for (int_fast32_t ion = 0; ion < NUMBER_OF_IONNAMES; ++ion) {
      ...
      if (cell_magnetic_field.size() > 0) {
        _voronoi_densityvalues[i].set_magnetic_field(cell_magnetic_field[i]);
      }
    }
 ...
 */
void CMacIonizeSnapshotDensityFunctionMHD::free() {
  if (_cartesian_grid) {
    for (uint_fast32_t ix = 0; ix < _ncell.x(); ++ix) {
      for (uint_fast32_t iy = 0; iy < _ncell.y(); ++iy) {
        delete[] _cartesian_grid[ix][iy];
 ...
 */
DensityValues CMacIonizeSnapshotDensityFunctionMHD::operator()(const Cell &cell) {
