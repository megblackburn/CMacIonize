...
/**
 * @file MagnetoHydroIntegrator.hpp
 *
 * @brief Class that performs the magneto-hydrodynamical integration.
 *
 * @author Meg Blackburn (mgb27@st-andrews.ac.uk)
 */
#ifndef MAGNETOHYDROINTEGRATOR_HPP
#define MAGNETOHYDROINTEGRATOR_HPP

#include "BondiProfile.hpp"
#include "DensityGridMHD.hpp"
#include "DensityGridTraversalJobMarket.hpp"
#include "GradientCalculatorMHD.hpp"
#include "MagnetoHydroBoundaryConditions.hpp"
#include "InternalMagnetoHydroUnits.hpp"
#include "ParameterFile.hpp"
#include "PhysicalConstants.hpp"
#include "RiemannSolverFactoryMHD.hpp"
...
#define magnetohydro_start_parallel_timing_block()                                    \
  serial_timer.stop();                                                         \
  parallel_timer.start();

/*! @brief Stop the parallel time timer and start the serial time timer. */
#define magnetohydro_stop_parallel_timing_block()                                     \
  parallel_timer.stop();                                                       \
  serial_timer.start();
...
private:
  ...
  const MagnetoHydroBoundaryConditionType _boundaries[6];
   ...
   */
  static MagnetoHydroBoundaryConditionType get_boundary_type(std::string type) {
    if (type == "periodic") {
      return MAGNETOHYDRO_BOUNDARY_PERIODIC;
    } else if (type == "reflective") {
      return MAGNETOHYDRO_BOUNDARY_REFLECTIVE;
    } else if (type == "inflow") {
      return MAGNETOHYDRO_BOUNDARY_INFLOW;
    } else if (type == "outflow") {
      return MAGNETOHYDRO_BOUNDARY_OUTFLOW;
    } else if (type == "bondi") {
      return MAGNETOHYDRO_BOUNDARY_BONDI;
    } else {
      cmac_error("Unknown boundary condition type: %s!", type.c_str());
      return MAGNETOHYDRO_BOUNDARY_INVALID;
    }
  }
...
public:
   ...
   */
  class MagnetoHydroFluxComputation {
  private:
    /*! @brief Reference to the underlying MagnetoHydroIntegrator. */
    const MagnetoHydroIntegrator &_magnetohydro_integrator;

    /*! @brief Reference to the grid. */
    const DensityGridMHD &_grid;
...
// make sure all densities and pressures are physical
      ...
      solver.solve_for_flux_MHD(rhoL_prime, uL_prime, PL_prime, rhoR_prime,
  ...
  public:
     ...
     * @brief Constructor.
     *
     * @param magnetohydro_integrator Reference to the underlying MagnetoHydroIntegrator.
     ...
     */
    inline MagnetoHydroFluxComputation(const MagnetoHydroIntegrator &magnetohydro_integrator,
                                const DensityGridMHD &grid,
                                const DensityGridMHD::iterator &grid_end,
                                double timestep)
        : _magnetohydro_integrator(magnetohydro_integrator), _grid(grid), _grid_end(grid_end),
          _timestep(timestep) {}
    ...
    inline void operator()(DensityGridMHD::iterator &cell) {
      ...
      for (auto ngbit = ngbs.begin(); ngbit != ngbs.end(); ++ngbit) {
        ...
        const double surface_area =
            _magnetohydro_integrator._magnetohydro_units
        ...
        const CoordinateVector<> dR =
            ...
            _magnetohydro_integrator._magnetohydro_units
                ->get_unit_internal_value< QUANTITY_LENGTH >();
        const double rinv = _magnetohydro_integrator._magnetohydro_units
        ...
        if (ngb != _grid_end) {
          ...
          vframe = _magnetohydro_integrator._magnetohydro_units
          ...
          for (uint_fast8_t i = 0; i < 3; ++i) {
            if ((normal[i] < 0. && _magnetohydro_integrator._boundaries[2 * i] ==
                                       MAGNETOHYDRO_BOUNDARY_REFLECTIVE) ||
                (normal[i] > 0. && _magnetohydro_integrator._boundaries[2 * i + 1] ==
                                       MAGNETOHYDRO_BOUNDARY_REFLECTIVE)) {
              uR[i] = -uR[i];
              ...
              gradPR[i] = -gradPR[i];
                        ...
                        _magnetohydro_integrator._boundaries[2 * i] ==
                            MAGNETOHYDRO_BOUNDARY_BONDI) ||
                       (normal[i] > 0. &&
                        _magnetohydro_integrator._boundaries[2 * i + 1] ==
                            MAGNETOHYDRO_BOUNDARY_BONDI)) {
              double nfrac;
              _magnetohydro_integrator._bondi_profile->get_hydrodynamic_variables(
        ...
        const double fluxfac = limit_flux(
            ...
            pL2 * rhoL > _magnetohydro_integrator._gamma * mL2 * PL,
            pR2 * rhoR > _magnetohydro_integrator._gamma * mR2 * PR,
            _magnetohydro_integrator._gamma == 1.);
        mflux *= fluxfac;
        pflux *= fluxfac;
        Eflux *= fluxfac;
#endif
...
private:
  ...
  inline const BondiProfile *get_bondi_profile(ParameterFile &params) {
    ...
    const std::string xlow =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary x low");
    const std::string xhigh =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary x high");
    const std::string ylow =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary y low");
    const std::string yhigh =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary y high");
    const std::string zlow =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary z low");
    const std::string zhigh =
        params.get_value< std::string >("MagnetohydroIntegrator:boundary z high");
...
public:
   ...
   * conditions.
   */
  inline MagnetoHydroIntegrator(const double gamma, const bool do_radiative_heating,
      ...
      : _gamma(gamma), _gm1(_gamma - 1.), _gm1_inv(1. / _gm1),
        ...
        _n_conversion_factor(1. / PhysicalConstants::get_physical_constant(
                                      PHYSICALCONSTANT_PROTON_MASS)),
        _magnetohydro_units(nullptr),
        _solver(RiemannSolverFactoryMHD::generate(riemann_solver_type, gamma)),
    ...
    if (_boundaries[0] == MAGNETOHYDRO_BOUNDARY_PERIODIC) {
      if (_boundaries[1] != MAGNETOHYDRO_BOUNDARY_PERIODIC) {
    ...
    if (_boundaries[2] == MAGNETOHYDRO_BOUNDARY_PERIODIC) {
      if (_boundaries[3] != MAGNETOHYDRO_BOUNDARY_PERIODIC) {
    ...
    if (_boundaries[4] == MAGNETOHYDRO_BOUNDARY_PERIODIC) {
      if (_boundaries[5] != MAGNETOHYDRO_BOUNDARY_PERIODIC) {
    ...
    if (_boundaries[0] == MAGNETOHYDRO_BOUNDARY_BONDI && _bondi_profile == nullptr) {
      cmac_error(
          "Bondi inflow boundaries only work if a Bondi profile is given.");
    }

    _magnetohydro_units = new InternalMagnetohydroUnits(1., 1., 1.);
  }
   ...
   * @param params ParameterFile to read from.
   */
  inline MagnetoHydroIntegrator(const SimulationBox &simulation_box,
                         ParameterFile &params)
      : MagnetoHydroIntegrator(
            params.get_value< double >("MagnetohydroIntegrator:polytropic index",
                                       5. / 3.),
            params.get_value< bool >("MagnetohydroIntegrator:radiative heating", true),
            params.get_value< bool >("MagnetohydroIntegrator:radiative cooling",
                                     false),
            params.get_value< double >("MagnetohydroIntegrator:CFL constant", 0.2),
            params.get_value< std::string >(
                "MagnetohydroIntegrator:Riemann solver type", "HLLD"),
            params.get_physical_value< QUANTITY_TEMPERATURE >(
                "MagnetohydroIntegrator:neutral temperature", "100. K"),
            params.get_physical_value< QUANTITY_TEMPERATURE >(
                "MagnetohydroIntegrator:ionised temperature", "1.e4 K"),
            params.get_physical_value< QUANTITY_TEMPERATURE >(
                "MagnetohydroIntegrator:shock temperature", "3.e4 K"),
            params.get_physical_value< QUANTITY_VELOCITY >(
                "MagnetohydroIntegrator:maximum velocity", "1.e99 m s^-1"),
            params.get_value< std::string >("MagnetohydroIntegrator:boundary x low",
                                            "reflective"),
            params.get_value< std::string >("MagnetohydroIntegrator:boundary x high",
                                            "reflective"),
  ...
  inline void update_energy_variables(IonizationVariables &ionization_variables,
      ...
      const double B2 = magnetohydro_variables.get_primitives_magnetic_field().norm2(); // mgb edit 24.04.2026
      const double magnetic_energy = B2/(2 *_mu0 * inverse_volume);
      ...
      double new_pressure =
          (_gamma-1.) * inverse_volume * (new_energy - kinetic_energy-magnetic_energy); // mgb edit 24.04.2026
      double new_temperature = _T_conversion_factor * mean_molecular_mass *
                               new_pressure * inverse_density;

      new_temperature = _magnetohydro_units->convert_to_SI_units<QUANTITY_TEMPERATURE>(new_temperature);
   ...
   */
  inline ~MagnetohydroIntegrator() {
    delete _solver;
    if (_bondi_profile != nullptr) {
      delete _bondi_profile;
    }
    if (_magnetohydro_units != nullptr) {
      delete _magnetohydro_units;
    }
  }
   ...
   *
   * @return InternalMagnetohydroUnits.
   */
  inline const InternalMMagnetohydroUnits *get_internal_units() const {
    return _magnetohydro_units;
  }

  /**
   * @brief Initialize the hydro variables for the given DensityGridMHD.
   *
   * Note that after this step all hydro variables will have been converted into
   * internal units.
   *
   * @param grid DensityGridMHD to operate on.
   */
  inline void initialize_hydro_variables(DensityGridMHD &grid) {
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      ...
      const double B_scalar = it.get_hydro_variables().get_B_scalar(); // mgb edit 24.04.2026
      const CoordinateVector<> B_field = it.get_hydro_variables().get_magnetic_field(); // mgb edit 
      
      it.get_hydro_variables().set_primitives_B_scalar(B_scalar); // mgb edit 24.04.2026
      it.get_hydro_variables().set_primitives_magnetic_field(B_field); // mgb edit

      it.get_hydro_variables().set_conserved_B_scalar(B_scalar*volume); // mgb edit
      it.get_hydro_variables().set_conserved_magnetic_field(B_field*volume); // mgb edit

      const double ekin = CoordinateVector<>::dot_product(velocity, momentum);
      if (_gamma > 1.) {
        // E = V*(rho*u + 0.5*rho*v^2) = V*(P/(gamma-1) + 0.5*rho*v^2)
        const double total_energy = volume * pressure * _gm1_inv + 0.5 * ekin + B_field.norm2()/(2 *_mu0) * volume; // mgb edit 24.04.2026
...
#else
    delete _magnetohydro_units;
    _magnetohydro_units = new InternalMagnetohydroUnits(avg_box_size, avg_rho, avg_P);
#endif

    const double velocity_unit_internal =
        _magnetohydro_units->get_unit_internal_value< QUANTITY_VELOCITY >();
    ...
    const double velocity_unit_SI =
        _magnetohydro_units->get_unit_SI_value< QUANTITY_VELOCITY >();
    ...
    _n_conversion_factor *=
        _magnetohydro_units->get_unit_SI_value< QUANTITY_DENSITY >();
    // rescale all magnetohydro variables to the internal unit system
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      it.get_hydro_variables().primitives(0) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_DENSITY >(
              it.get_hydro_variables().primitives(0));
      it.get_hydro_variables().primitives(1) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_VELOCITY >(
              it.get_hydro_variables().primitives(1));
      ...
      it.get_hydro_variables().primitives(4) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_PRESSURE >(
              it.get_hydro_variables().primitives(4));

      it.get_hydro_variables().conserved(0) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_MASS >(
              it.get_hydro_variables().conserved(0));
      it.get_hydro_variables().conserved(1) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_MOMENTUM >(
              it.get_hydro_variables().conserved(1));
      ...
      it.get_hydro_variables().conserved(4) =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_ENERGY >(
              it.get_hydro_variables().conserved(4));
    }

    grid.set_grid_velocity(
        _gamma, _magnetohydro_units->get_unit_SI_value< QUANTITY_VELOCITY >());
    _max_velocity =
        _magnetohydro_units->convert_to_internal_units< QUANTITY_VELOCITY >(
            _max_velocity);
  }
   ...
   */
  inline double get_soundspeed(const DensityGridMHD::iterator &cell) const {
    if (_gamma > 1.) {
   ...
   * integration.
   *
   * @param grid DensityGridMHD on which to operate.
   * @return Maximal system time step that yields a stable integration (in s).
   */
  inline double get_maximal_timestep(DensityGridMHD &grid) const {
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      ...
      const double V =
          _magnetohydro_units->convert_to_internal_units< QUANTITY_VOLUME >(
              it.get_volume());
    ...
    return _magnetohydro_units->convert_to_SI_units< QUANTITY_TIME >(_CFL_constant *
                                                              dtmin);
  }
   ...
   * the algorithm.
   */
  inline void do_magnetohydro_step(DensityGridMHD &grid, double timestep,
                            Timer &serial_timer, Timer &parallel_timer) const {

    const double internal_timestep =
        _magnetohydro_units->convert_to_internal_units< QUANTITY_TIME >(timestep);

    const DensityGridMHD::iterator grid_end = grid.end();
    std::pair< cellsize_t, cellsize_t > block =
        std::make_pair(0, grid.get_number_of_cells());

#ifndef NO_SECOND_ORDER
    // if second order scheme: compute gradients for primitive variables
    GradientCalculatorMHD::GradientComputation gradient_computation(
        _boundaries, grid_end,
        _magnetohydro_units->get_unit_internal_value< QUANTITY_LENGTH >(),
        _magnetohydro_units->get_unit_internal_value< QUANTITY_SURFACE_AREA >(),
        _magnetohydro_units->get_unit_SI_value< QUANTITY_VOLUME >());
    WorkDistributor<
        DensityGridTraversalJobMarket<
            GradientCalculatorMHD::GradientComputation >,
        DensityGridTraversalJob< GradientCalculatorMHD::GradientComputation > >
        gradient_workers;
    DensityGridTraversalJobMarket< GradientCalculatorMHD::GradientComputation >
        gradient_jobs(grid, gradient_computation, block);
    magnetohydro_start_parallel_timing_block();
    gradient_workers.do_in_parallel(gradient_jobs);
    magnetohydro_stop_parallel_timing_block();
#endif
    ...
    for (auto it = grid.begin(); it != grid.end(); ++it) {
      ...
      if (rho > 0. && !std::isinf(rho_inv)) {
        ...
        const CoordinateVector<> a =
            _magnetohydro_units->convert_to_internal_units< QUANTITY_ACCELERATION >(
    ...
    MagnetoHydroFluxComputation magnetohydro_flux_computation(*this, grid, grid_end,
                                                internal_timestep);

    WorkDistributor< DensityGridTraversalJobMarket< MagnetoHydroFluxComputation >,
                     DensityGridTraversalJob< MagnetoHydroFluxComputation > >
        workers;
    DensityGridTraversalJobMarket< MagnetoHydroFluxComputation > jobs(
        grid, magnetohydro_flux_computation, block);
    grid.reset_access_flags();
    magnetohydro_start_parallel_timing_block();
    workers.do_in_parallel(jobs);
    magnetohydro_stop_parallel_timing_block();
    if (!grid.check_access()) {
      cmac_error("Access error in magnetohydro flux exchange loop!");
    }
    ...
    if (_do_explicit_heating) {
      for (auto it = grid.begin(); it != grid.end(); ++it) {
        ...
        MagnetoHydroVariables &magnetohydro_variables = it.get_hydro_variables();
        ...
        double vol = _magnetohydro_units->convert_to_internal_units<QUANTITY_VOLUME>(it.get_volume());
        dE = _magnetohydro_units->convert_to_internal_units<QUANTITY_ENERGY>(dE);
...
#endif // MAGNETOHYDROINTEGRATOR_HPP
