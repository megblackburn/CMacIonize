...
public:
  ...
  inline StarburstDensityFunction(ParameterFile &params)
      : _disc_z(params.get_physical_value< QUANTITY_LENGTH >("DensityFunction:disc z", "0. m")),
        ...
        _velocity_dispersion(params.get_physical_value< QUANTITY_VELOCITY >("DensityFunction:velocity_dispersion", "0. km s^-1")),
        _random_generator() 
  {
      ...
      r_m = params.get_physical_value< QUANTITY_LENGTH >("ExternalPotential:patch radius", "1.5 kpc");
      ...
      _R_disk_gas   = params.get_physical_value< QUANTITY_LENGTH >("DensityFunction:gas_scale_radius", "1.6 kpc");
      _n_peak_disk  = params.get_physical_value< QUANTITY_NUMBER_DENSITY >("DensityFunction:peak_midplane_density", "200.0 cm^-3"); 
      _n_halo_floor = params.get_physical_value< QUANTITY_NUMBER_DENSITY >("DensityFunction:halo_density_floor", "1.0e-3 cm^-3");
      _T_halo_floor = params.get_physical_value< QUANTITY_TEMPERATURE >("DensityFunction:halo_temperature_floor", "2.0e6 K");
  }
  ...
  double get_cgols_potential_value(const double x, const double y, const double z) const {
    ...
    const double r  = std::sqrt(R2 + z*z);
  ...
  /**
   * @brief Calculates rotation curve velocities
   */
  ...
  virtual DensityValues operator()(const Cell &cell) override {
    ...
    if (_mode == MODE_PATCH) {
        const double R_fixed = r_m; // Safe 1 kpc conversion scale
        x = R_fixed;
        y = 0.0;
        R = R_fixed;
    }
