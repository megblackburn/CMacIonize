  ...
  inline BurstyPhotonSourceDistribution(
    ...
    if (output_sources) {
      ...
      if (_output_file2->tellp() == 0) {
        *_output_file2 << "simulation time (s)\ttotal time (s)\tlum (s^-1)\tnumsne\tSFR_base (Msol Myr-1)\tSFR_KS (Msol Myr^-1 kpc^-2)\tSFR_KS (kg s^-1 kpc^-2)\tM_init\tM_init_unscaled_for_sfr\n";
      }
      _output_file2->flush();
