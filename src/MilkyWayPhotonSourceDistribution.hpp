...
/**
 * @file MilkyWayPhotonSourceDistribution.hpp
 *
 * @brief Bursty PhotonSourceDistribution.
 *
 * @author Meg Blackburn (mgb27@st-andrews.ac.uk)
 */
#ifndef MILKYWAYPHOTONSOURCEDISTRIBUTION_HPP
#define MILKYWAYPHOTONSOURCEDISTRIBUTION_HPP
 ...
 */
class MilkyWayPhotonSourceDistribution : public PhotonSourceDistribution {
private:
...
// const double bursty_star_formation_rate; 
 ...
 const double _start_type1a_sne;
 /*! what time to start the Type Ia supernovae */

 const bool _delay_supernovae;
 /*! delay the onset of supernovae */
...
public:
   ...
   */
  inline MilkyWayPhotonSourceDistribution(
      ...
      const double start_type1a_sne, // mgb edit 16.07.2026
      const bool delay_supernovae, // mgb edit 06.07.2026
    //  const bool moving_sources_flag, // mgb 16.10.2025
      ...
      : _star_formation_rate(star_formation_rate), _bursty_sfr_flag(bursty_sfr_flag), _KS_scaling_flag(KS_scaling_flag), _length_of_burst(length_of_burst), _time_of_burst_peak(time_of_burst_peak),
        ...
        _M_init(M_init), _M_init_unscaled_for_sfr(M_init_unscaled_for_sfr), _use_mw_sfh(use_mw_sfh), _start_mw_sfh_time(start_mw_sfh_time), _start_type1a_sne(start_type1a_sne), _delay_supernovae(delay_supernovae),
    ...
    */
    if (output_sources) {
        ...
        if (_output_file2->tellp() == 0) {
          *_output_file2 << "simulation time (s)\tlum (s^-1)\tnumsne\tSFR_base (Msol Myr-1)\tM_gen (Msol)\tSFR (kg s^-1 kpc^-2)\tM_init\tM_init_unscaled_for_sfr\n";
        }
        _output_file2->flush();
   ...
   */
  MilkyWayPhotonSourceDistribution(ParameterFile &params, Log *log = nullptr)
      : MilkyWayPhotonSourceDistribution(
            ...
            params.get_physical_value< QUANTITY_TIME >("PhotonSourceDistribution:start type1a sne", "0.0 Myr"), // mgb edit 16.07.2026
            params.get_value<bool>("PhotonSourceDistribution:delay supernovae", false),
   ...
   */
  virtual ~MilkyWayPhotonSourceDistribution() {}
...
// Stellar Sources
    if (_total_time - _last_sf > _update_interval) {
      ...
      if (_KS_scaling_flag == true){
         mass_to_generate = _update_interval*star_formation_rate/unit_Msol*0.207*(std::pow(running_mass_unscaled_for_sfr/init_running_mass_unscaled_for_sfr, _kennicutt_schmidt_index)); // replaced 1.988e30 with unit_Msol mgb edit 12.11.2025 
      } else {
         mass_to_generate = _update_interval*star_formation_rate/unit_Msol*0.207; // mgb ed0it 25.05.2026
      }

      if (_output_file2 != nullptr) {
        double totallum = get_total_luminosity();
        *_output_file2 << _total_time << "\t" << totallum << "\t" << _num_sne << "\t" << ((star_formation_rate*unit_Myr)/(unit_Msol * area_kpc)) << "\t" << mass_to_generate << "\t" <<  (star_formation_rate/area_kpc) << "\t" << init_running_mass << "\t" << init_running_mass_unscaled_for_sfr << "\n"; // output the SFR in Msol Myr^-1 kpc^-2 and in kg s^-1 kpc^-2
        _output_file2->flush();

      }

       std::cout << "SHOULD BE GENERATING " << mass_to_generate - _excess_mass<< std::endl;
       std::cout << " _update_interval: " << _update_interval << ", SFR: " << star_formation_rate << ", mass_to_generate: " << mass_to_generate << std::endl;
  ...
  // Type II supernovae: ///
    ...
    while (i < _source_lifetimes.size()) {
      ...
      if (_source_lifetimes[i] <= 0.) { // Star Dying so injects supernovae after erasing O star: stellar feedback would copy this but happen every timestep
        ...
        if (_delay_supernovae == true) {
          if (_total_time >= _start_mw_sfh_time){
            _to_do_feedback.push_back(_source_positions[i]);
            _source_positions.erase(_source_positions.begin() + i);
            _source_lifetimes.erase(_source_lifetimes.begin() + i);
            _source_luminosities.erase(_source_luminosities.begin() + i);
...
// --------------------------------------
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    restart_writer.write(_use_mw_sfh),
    restart_writer.write(_start_mw_sfh_time),
    restart_writer.write(_start_type1a_sne),
    restart_writer.write(_delay_supernovae),
   ...
   */
  inline MilkyWayPhotonSourceDistribution(RestartReader &restart_reader)
      ...
      //  _moving_sources_flag(restart_reader.read< bool >())
        ...
        _use_mw_sfh(restart_reader.read< bool >()),
        _start_mw_sfh_time(restart_reader.read<double>()),
...
#endif // MILKYWAYPHOTONSOURCEDISTRIBUTION_HPP
