 ...
 */
inline void wrap_milky_way_source_position(
...
public:
  ...
  MilkyWayPhotonSourceDistribution(ParameterFile &params, Log *log = nullptr)
      : MilkyWayPhotonSourceDistribution(
            ...
            params.get_value< double >("PhotonSourceDistribution:clustering factor", 1.0), // mgb edit 11.11.2025x
  ...
  // Type II supernovae: ///
    ...
    while (i < _source_lifetimes.size()) {
      ...
      if (_source_lifetimes[i] <= 0.) { // Star Dying so injects supernovae after erasing O star: stellar feedback would copy this but happen every timestep
        ...
        _snapshot_supernova_types.push_back(2); // Type II supernovae
        _num_sne = _num_sne + 1;
        updated = true;
...
// Stellar Sources
    if (_total_time - _last_sf >= _update_interval) {
      ...
      if (_KS_scaling_flag == true){
         mass_to_generate = star_formation_interval*star_formation_rate/unit_Msol*0.207*(std::pow(running_mass/init_running_mass, _kennicutt_schmidt_index)); // replaced 1.988e30 with unit_Msol mgb edit 12.11.2025 
      } else {
         mass_to_generate = star_formation_interval*star_formation_rate/unit_Msol*0.207; // mgb ed0it 25.05.2026
      }
      ...
      if (_output_file2 != nullptr) {
       ...
       std::cout << " star_formation_interval: " << star_formation_interval << ", SFR: " << star_formation_rate << ", mass_to_generate: " << mass_to_generate << std::endl;
  ...
  virtual void float_sources(
    ...
    while (i < _source_positions.size()) {
      ...
      wrap_milky_way_source_position(_source_positions[i], box,
                                         periodicity);
      if (!grid_creator->get_box().inside(_source_positions[i])) {
        if (_log != nullptr) {
          _log->write_warning(
...
// --------------------------------------
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    }
    {
      ...
      for (const int type : _snapshot_supernova_types) {
