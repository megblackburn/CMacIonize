...
// const double bursty_star_formation_rate; 
  ...
  /*! @brief Time of burst Start (in Myr). */
  const double _time_of_burst_start;
...
public:
  ...
  inline BurstyPhotonSourceDistribution(
      ...
      const double time_of_burst_start, // mgb edit 19.09.2025
      ...
      Log *log=nullptr)
      : _star_formation_rate(star_formation_rate), _bursty_sfr_flag(bursty_sfr_flag), _length_of_burst(length_of_burst), _time_of_burst_start(time_of_burst_start), _burst_amplitude(burst_amplitude),
  ...
  BurstyPhotonSourceDistribution(ParameterFile &params, Log *log = nullptr)
      : BurstyPhotonSourceDistribution(
            ...
            params.get_physical_value< QUANTITY_TIME_OF_BURST_START >(
                "PhotonSourceDistribution:time of burst start", "80.0 Myr"), // edit mgb 19.09.2025 - may need to change unit to yr
   ...
   */
   virtual bool update(DensitySubGridCreator< HydroDensitySubGrid > *grid_creator, double actual_timestep) override {
    ...
    int should_have_done = int(4*area_kpc*_total_time/3.15576e13); // taken out *0 mgb 09.10.2025
 ...
 // top hat 
  //    double _bursty_star_formation_rate = _star_formation_rate;
    ...
    //  if ((_total_time >= _time_of_burst_start) && (_total_time <= (_time_of_burst_start + _length_of_burst))) {
     ...
     if (_bursty_sfr_flag == true) {
      _bursty_star_formation_rate = _star_formation_rate + (_burst_amplitude*_star_formation_rate - _star_formation_rate) * exp(-pow((_total_time - _time_of_burst_start), 2)/_length_of_burst); // edit mgb 19.09.2025
     } else {
      _bursty_star_formation_rate = _star_formation_rate;
     }
...
// --------------------------------------
  ...
  virtual void write_restart_file(RestartWriter &restart_writer) const {
    ...
    restart_writer.write(_time_of_burst_start); // mgb edit 19.09.
  ...
  inline BurstyPhotonSourceDistribution(RestartReader &restart_reader)
      : _star_formation_rate(restart_reader.read< double >()),
        ...
        _time_of_burst_start(restart_reader.read< double >()), // mgb edit 19.09.2025
