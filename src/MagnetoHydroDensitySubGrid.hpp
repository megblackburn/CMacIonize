...
public:
  ...
  inline void update_conserved_variables(const double timestep) {
    ...
    for (int_fast32_t i = 0; i < tot_num_cells; ++i) {
      ...
      double current_mass = _hydro_variables[i].get_conserved_mass();

      if (current_Eint <= 0.){
        std::cout << "FATAL Eint <= 0., Eint:  " << current_Eint << std::endl;
      }
     // double eint_flux_delta = _hydro_variables[i].delta_conserved(9) * timestep;
     // double final_Eint = current_Eint + energy_term + eint_flux_delta;
     ...
     // double final_Etot = current_Etot + grav_work + energy_term + flux_delta;
      ...
      _hydro_variables[i].conserved(4) += _hydro_variables[i].get_energy_term(); // mgb comment 19.05.2026 supernova energy?
      _hydro_variables[i].conserved(9) += _hydro_variables[i].get_energy_term();
      _hydro_variables[i].set_energy_term(0.);

      

      /*if (_hydro_variables[i].conserved(9)+(_hydro_variables[i].delta_conserved(9) * timestep) < 0.){
        //std::cout<<"POTENTIAL FATAL: Eint update is driving it negative, reverting to mass limit." << std::endl;
        if (_hydro_variables[i].conserved(0)+(_hydro_variables[i].delta_conserved(0) * timestep) >= 0) {
          double specific_eint = _hydro_variables[i].get_conserved_internal_energy()/_hydro_variables[i].get_conserved_mass();
          double delta_mass = _hydro_variables[i].delta_conserved(0);
          _hydro_variables[i].delta_conserved(9) = delta_mass * specific_eint;
        } else {
          double specific_eint = _hydro_variables[i].get_conserved_internal_energy()/_hydro_variables[i].get_conserved_mass();
          double delta_mass = _hydro_variables[i].delta_conserved(0);
          double limit_factor = 1e-3;
          _hydro_variables[i].delta_conserved(9) = delta_mass * specific_eint * limit_factor;
          _hydro_variables[i].delta_conserved(0) *= limit_factor;

        } 
      }*/

      double eint_flux_delta = _hydro_variables[i].delta_conserved(9) * timestep;
      double mass_flux_delta = _hydro_variables[i].delta_conserved(0) * timestep;
     ...
     //   _hydro_variables[i].primitive_gradients(j) = CoordinateVector<>(0.); // mgb edit 28.05.2026
        _primitive_variable_limiters[20 * i + 2 * j] = DBL_MAX;
        _primitive_variable_limiters[20 * i + 2 * j + 1] = -DBL_MAX;
      }

      // DUAL ENERGY LIMIT: mgb edit 29.05.2026
      double E_total = _hydro_variables[i].conserved(4);
      double e_dual  = _hydro_variables[i].conserved(9);
      double rho     = _hydro_variables[i].conserved(0);

      double _mu0 = PhysicalConstants::get_physical_constant(PHYSICALCONSTANT_MAGNETC_PERMEABILITY);

      double vx_c = _hydro_variables[i].conserved(1) / rho;
      double vy_c = _hydro_variables[i].conserved(2) / rho;
      double vz_c = _hydro_variables[i].conserved(3) / rho;
      double e_kin = 0.5 * rho * (vx_c*vx_c + vy_c*vy_c + vz_c*vz_c);

      double Bx_c = _hydro_variables[i].conserved(5);
      double By_c = _hydro_variables[i].conserved(6);
      double Bz_c = _hydro_variables[i].conserved(7);
      double e_mag = (Bx_c*Bx_c + By_c*By_c + Bz_c*Bz_c) / (2.0 * _mu0);

      double e_subtraction = E_total - e_kin - e_mag;

      const double eta1 = 0.001; 

      if (e_subtraction / E_total < eta1 || e_subtraction <= 0.0) {

          _hydro_variables[i].conserved(4) = e_dual + e_kin + e_mag;
      } 
      else {

          _hydro_variables[i].conserved(9) = e_subtraction;
      }

      
      if (_hydro_variables[i].get_conserved_total_energy() <= 0.0  || _hydro_variables[i].get_conserved_internal_energy() <0.0 || _hydro_variables[i].get_conserved_total_energy() < _hydro_variables[i].get_conserved_internal_energy())  {
        std::cout << "[FATAL ENERGY LOSS] Cell Index: " << i << "\n"
                  << "  Initial Etot:  " << current_Etot << "\n"
                  << "  Initial Eint: " << current_Eint << "\n"
                  << "  Eint Flux Delta: " << eint_flux_delta << "\n"
                  << "  Final Eint: " << _hydro_variables[i].get_conserved_internal_energy() << "\n"
                  << "  Grav Work:     " << grav_work << "\n"
                  << "  Energy Term:   " << energy_term << "\n"
                  << "  Flux Delta:    " << flux_delta << "\n"
                  << "  Initial Mass:  " << current_mass << "\n"
                  << "  Final Mass:          " << _hydro_variables[i].get_conserved_mass() << "\n"
                  << "  Mass Flux Delta: " << mass_flux_delta << "\n"
                  << "  Result:        " << _hydro_variables[i].get_conserved_total_energy() << std::endl;
      }
    
      double total_magnetic_energy = 0.0;
      double total_divergence_energy = 0.0; // Energy trapped in the B_scalar (ψ) field

      const int_fast32_t tot_num_cells = _number_of_cells[0] * _number_of_cells[1] * _number_of_cells[2];
      for (int_fast32_t i = 0; i < tot_num_cells; ++i) {
          double Bx_c = _hydro_variables[i].conserved(5);
          double By_c = _hydro_variables[i].conserved(6);
          double Bz_c = _hydro_variables[i].conserved(7);
          double psi  = _hydro_variables[i].conserved(8); // Your B_scalar

          total_magnetic_energy += (Bx_c*Bx_c + By_c*By_c + Bz_c*Bz_c) / (2.0 * _mu0);
          
          total_divergence_energy += (psi * psi) / (2.0 * _mu0); 
      }

      double cleaning_ratio = 0.0;
      if (total_magnetic_energy > 0.0) {
          cleaning_ratio = total_divergence_energy / total_magnetic_energy;
      }

      if (cleaning_ratio > 0.10) {
          std::cout << "WARNING: Dedner field is clearing more than 10% of the magnetic profile!\n"
                    << "  Current Monopole Energy Ratio: " << (cleaning_ratio * 100.0) << " %\n"
                    << "  Total B-Field Energy: " << total_magnetic_energy << "\n"
                    << "  Total Psi Field Energy: " << total_divergence_energy << std::endl;
      }
    ...
    if ((std::isnan(_hydro_variables[i].get_conserved_total_energy())) || (_hydro_variables[i].get_conserved_total_energy() == 0)) {
        std::cerr << "Total energy is NaN or 0. after conserved variable update for cell in MagnetoHydroDensitySubGrid" << i << ", mass: " << _hydro_variables[i].get_conserved_mass() << ", momentum: (" << _hydro_variables[i].get_conserved_momentum().x() << ", " << _hydro_variables[i].get_conserved_momentum().y() << ", " << _hydro_variables[i].get_conserved_momentum().z() << ")" << ", Etot: " << _hydro_variables[i].get_conserved_total_energy() << ", Eint: " << _hydro_variables[i].get_conserved_internal_energy() << std::endl;
  ...
  inline void inner_gradient_sweep(const MagnetoHydro &hydro) {
     ...
     const int_fast32_t tot_cells = _number_of_cells[0] * _number_of_cells[3];
    for (int_fast32_t cell = 0; cell < tot_cells; ++cell) {
      for (int_fast8_t j = 0; j < 10; ++j) {
         _hydro_variables[cell].primitive_gradients(j) = CoordinateVector<>(0.); // mgb edit 28.05.2026
      }
    }
    // we do three separate sweeps: one for every coordinate direction
    for (int_fast32_t ix = 0; ix < _number_of_cells[0] - 1; ++ix) {
