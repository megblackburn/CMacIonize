...
#include "HydroBoundaryTracerss.hpp"
...
private:
  ...
  /*! @brief Enable cold gas tracking */
...
public:
  ...
  inline void set_primitive_variables(HydroVariables &hydro_state,
    ...
    if (_trace_initial_neutral_flag == true){
      const double X_og = hydro_state.get_conserved_initial_cold_gas_scalar_field()/ density;
      const double X_cool = hydro_state.get_conserved_cooled_gas_scalar_field() / density;
      
      hydro_state.set_primitives_cooled_gas_scalar_field(X_cool);
      hydro_state.set_primitives_initial_cold_gas_scalar_field(X_og);
    } else {
      hydro_state.set_primitives_initial_cold_gas_scalar_field(0.0);
      hydro_state.set_primitives_cooled_gas_scalar_field(0.0);
    }
  ...
  inline void set_conserved_variables(HydroVariables &state,
    ...
    // mgb edit 12.06.2026
    if (_trace_initial_neutral_flag == true){
      const double X_og_dens = state.get_primitives_initial_cold_gas_scalar_field() * state.get_primitives_density();
      const double X_cool_dens = state.get_primitives_cooled_gas_scalar_field() * state.get_primitives_density();

      state.set_conserved_initial_cold_gas_scalar_field(X_og_dens);
      state.set_conserved_cooled_gas_scalar_field(X_cool_dens);
    } else {
      state.set_primitives_initial_cold_gas_scalar_field(0.0);
      state.set_primitives_cooled_gas_scalar_field(0.0);
    }
  }
  ...
  inline void do_flux_calculation(const uint_fast8_t i,
    ...
    double X_ogL = left_state.primitives(6) + halfdx * left_state.primitive_gradients(6)[i];
    double X_coolL = left_state.primitives(5) + halfdx * left_state.primitive_gradients(5)[i];

    double X_ogR = right_state.primitives(6) + halfdx * right_state.primitive_gradients(6)[i];
    double X_coolR = right_state.primitives(5) + halfdx * right_state.primitive_gradients(5)[i];
    ...
    X_ogL = limit(X_ogL, left_state.get_primitives_initial_cold_gas_scalar_field(), right_state.get_primitives_initial_cold_gas_scalar_field(), 0.5);
    X_coolL = limit(X_coolL, left_state.get_primitives_cooled_gas_scalar_field(), right_state.get_primitives_cooled_gas_scalar_field(), 0.5);

    X_ogR = limit(X_ogR, right_state.get_primitives_initial_cold_gas_scalar_field(), left_state.get_primitives_initial_cold_gas_scalar_field(), 0.5);
    X_coolR = limit(X_coolR, right_state.get_primitives_cooled_gas_scalar_field(), left_state.get_primitives_cooled_gas_scalar_field(), 0.5);
    ...
    left_state.delta_conserved(5) -= cool_flux;
    left_state.delta_conserved(6) -= orig_flux;
    ...
    right_state.delta_conserved(5) += cool_flux;
    right_state.delta_conserved(6) += orig_flux;
  }
  ...
  inline void do_ghost_flux_calculation(const uint_fast8_t i,
                                        ...
                                        HydroVariables &left_state,
                                        const HydroBoundaryTracers &boundary,
  ...
  inline void predict_primitive_variables(HydroVariables &state,
    ...
    const double X_og = state.get_primitives_initial_cold_gas_scalar_field();
    const double X_cool = state.get_primitives_cooled_gas_scalar_field();
    ...
    const double dXcdx = state.primitive_gradients(5).x();
    const double dXcdy = state.primitive_gradients(5).y();
    const double dXcdz = state.primitive_gradients(5).z();

    const double dXodx = state.primitive_gradients(6).x();
    const double dXody = state.primitive_gradients(6).y();
    const double dXodz = state.primitive_gradients(7).z();
    ...
    // mgb edit 12.06.2026
    state.set_primitives_cooled_gas_scalar_field(X_cool_new);
    state.set_primitives_initial_cold_gas_scalar_field(X_og_new);
  }
