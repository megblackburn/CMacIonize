...
int main(int argc, char **argv) {
  ...
  parser.add_option("task-based-rhd", 0, "Run a task-based RHD simulation.",
                    COMMANDLINEOPTION_NOARGUMENT, "false");
  parser.add_option("task-based-rhd-trace", 0, "Run a task-based RHD simulation with tracer fields.",
                    COMMANDLINEOPTION_NOARGUMENT, "false");
  ...
  TaskBasedRadiationMagnetoHydrodynamicsSimulation::add_command_line_parameters(
    ...
    TaskBasedRadiationHydrodynamicsSimulationTracers::add_command_line_parameters(
      parser); // mgb edit 16.06.2026
  ...
  if (parser.get_value< bool >("dusty-radiative-transfer")) {
    ...
    return TaskBasedRadiationMagnetoHydrodynamicsSimulation::do_simulation(
        parser, write_output, programtimer, log);
  } else if (parser.get_value< bool >("task-based-rhd-trace")) {

    if (comm.get_size() > 1) {
      cmac_error("MPI RMHD is not (yet) supported!");
    }
    return TaskBasedRadiationHydrodynamicsSimulationTracers::do_simulation(
        parser, write_output, programtimer, log);
  } else {
