...
/**
 * @file TaskBasedRadiationHydrodynamicsSimulationTracers.cpp
 *
 * @brief TaskBasedRadiationHydrodynamicsSimulationTracers implementation.
 *
 * @author Bert Vandenbroucke (bv7@st-andrews.ac.uk)
 */

#include "TaskBasedRadiationHydrodynamicsSimulationTracers.hpp"
...
#include "HydroBoundaryManagerTracers.hpp"
#include "HydroDensitySubGridTracers.hpp"
 ...
 * options.
 */
void TaskBasedRadiationHydrodynamicsSimulationTracers::add_command_line_parameters(
...
inline static void do_explicit_heat_cool(IonizationVariables &ionization_variables,
                              HydroVariablesTracers &hydro_variables,
 ...
 */
int TaskBasedRadiationHydrodynamicsSimulationTracers::do_simulation(
       ...
       while (igrid.value() < grid_creator->number_of_original_subgrids()) {
        ...
        if (this_igrid < grid_creator->number_of_original_subgrids()) {
          ...
          for (auto cellit = (*gridit).hydro_begin();
            ...
            HydroVariablesTracers hydro_variables = cellit.get_hydro_variables();
          ...
          for (auto cellit = (*gridit).hydro_begin();
           ...
           // hydro.set_primitive_variables(cellit.get_hydro_variables(), cellit.get_ionization_variables(), cellit.get_volume())
            ...
            if (_trace_initial_neutral_flag == true){
              const double temperature_after_cooling = cellit.get_ionization_variables().get_temperature();
              const double density = cellit.get_hydro_variables().get_primitives_density();
              double X_og = 0.0;
              double X_cool = 0.0;
              if (density>0.0){
                X_og = cellit.get_hydro_variables().get_conserved_initial_cold_gas_scalar_field()/density;
              }

              if (temperature_after_cooling <= _temperature_to_trace){
                if (X_og < 1.0){
                  X_cool = 1.0 - X_og;
                }
              } else {
              ...
              cellit.get_hydro_variables().set_conserved_cooled_gas_scalar_field(X_cool * density);
              cellit.get_hydro_variables().set_conserved_initial_cold_gas_scalar_field(X_og * density);
