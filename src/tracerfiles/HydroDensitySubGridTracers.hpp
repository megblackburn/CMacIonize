...
#include "DensitySubGridTracers.hpp"
...
private:
  ...
  HydroVariablesTracers *_hydro_variables;
...
public:
  ...
  inline HydroDensitySubGrid(const double *box,
    ...
    _hydro_variables = new HydroVariablesTracers[tot_ncell];
    _primitive_variable_limiters = new double[tot_ncell * 14];

    for (int_fast32_t i = 0; i < 7 * tot_ncell; ++i) {
  ...
  inline HydroDensitySubGrid(const HydroDensitySubGrid &original)
    ...
    for (int_fast32_t i = 0; i < 14 * tot_ncell; ++i) {
  ...
  inline void update_conserved_variables(const double timestep) {
    ...
    for (int_fast32_t i = 0; i < tot_num_cells; ++i) {
      ...
      for (int_fast8_t j = 0; j < 7; ++j) {
        ...
        _primitive_variable_limiters[14 * i + 2 * j] = DBL_MAX;
        _primitive_variable_limiters[14 * i + 2 * j + 1] = -DBL_MAX;
      }
  ...
  inline void apply_slope_limiter(const Hydro &hydro) {
    ...
    for (int_fast32_t i = 0; i < tot_num_cells; ++i) {
      hydro.apply_slope_limiter(_hydro_variables[i],
                                &_primitive_variable_limiters[14 * i],
                                _cell_size);
    }
  }
      ...
      for (int_fast32_t iy = 0; iy < _number_of_cells[1]; ++iy) {
        for (int_fast32_t iz = 0; iz < _number_of_cells[2]; ++iz) {
          ...
          hydro.do_gradient_calculation(
              ...
              _inv_cell_size[0], &_primitive_variable_limiters[14 * index000],
              &_primitive_variable_limiters[14 * index100]);
        }
      }
    }
    for (int_fast32_t ix = 0; ix < _number_of_cells[0]; ++ix) {
      for (int_fast32_t iy = 0; iy < _number_of_cells[1] - 1; ++iy) {
        for (int_fast32_t iz = 0; iz < _number_of_cells[2]; ++iz) {
          ...
          hydro.do_gradient_calculation(
              ...
              _inv_cell_size[1], &_primitive_variable_limiters[14 * index000],
              &_primitive_variable_limiters[14 * index010]);
        }
      }
    }
    for (int_fast32_t ix = 0; ix < _number_of_cells[0]; ++ix) {
      for (int_fast32_t iy = 0; iy < _number_of_cells[1]; ++iy) {
        for (int_fast32_t iz = 0; iz < _number_of_cells[2] - 1; ++iz) {
          ...
          hydro.do_gradient_calculation(
              ...
              _inv_cell_size[2], &_primitive_variable_limiters[14 * index000],
              &_primitive_variable_limiters[14 * index001]);
        }
      }
    }
  }
    ...
    for (int_fast32_t ic = 0; ic < column_length; ++ic) {
      for (int_fast32_t ir = 0; ir < row_length; ++ir) {
        ...
        hydro.do_gradient_calculation(
            ...
            &left_grid->_primitive_variable_limiters[14 * index_left],
            &right_grid->_primitive_variable_limiters[14 * index_right]);
      }
    }
  }
  ...
  virtual void initialize_hydro(const uint_fast32_t index,
    ...
    _hydro_variables[index].set_primitives_initial_cold_gas_scalar_field(values.get_initial_neutral_scalar_field());
    _hydro_variables[index].set_primitives_cooled_gas_scalar_field(values.get_neutral_scalar_field());
    }
  ...
  public:
     ...
     */
    inline const HydroVariablesTracers &get_hydro_variables() const {
     ...
     */
    inline HydroVariablesTracers &get_hydro_variables() {
      return _subgrid->_hydro_variables[_index];
    }
  ...
  inline HydroDensitySubGrid(RestartReader &restart_reader)
    ...
    const int_fast32_t number_of_cells =
        _number_of_cells[0] * _number_of_cells[1] * _number_of_cells[2];
    _hydro_variables = new HydroVariablesTracers[number_of_cells];
    for (int_fast32_t i = 0; i < number_of_cells; ++i) {
      _hydro_variables[i] = HydroVariablesTracers(restart_reader);
      _ionization_variables[i] = IonizationVariables(restart_reader);
    }
    _primitive_variable_limiters = new double[14 * number_of_cells];
    for (int_fast32_t i = 0; i < 7 * number_of_cells; ++i) {
      _primitive_variable_limiters[2 * i] = DBL_MAX;
      _primitive_variable_limiters[2 * i + 1] = -DBL_MAX;
    }
  }
